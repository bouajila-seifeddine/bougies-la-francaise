...
class ToolsCore
{
	...
	public static function getPath($id_category, $path = '', $link_on_the_item = false, $category_type = 'products', Context $context = null)
	{
		...
		if ($category_type === 'products')
		{
			...
			if ($interval)
			{
				...
				if (!empty($path)) {
					...
					$path = 
					'<div itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a itemprop="url" href="'.Tools::safeOutput($category_link).'">'.
					'<span itemprop="title">'.
		...
		else if ($category_type === 'CMS')
		{
			...
			if ($path != $category->name)
				...
				htmlentities($category->name, ENT_NOQUOTES, 'UTF-8').
				'</span></a></div>
				<span class="navigation-pipe">'.$pipe.'</span>'.$path;
			else
				$full_path = 
				'<div itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a itemprop="url" href="'.Tools::safeOutput($category_link).'">'.
				'<span itemprop="title">'.
	...
	*/
	public static function getFullPath($id_category, $end, $type_cat = 'products', Context $context = null)
	{
		...
		'<div itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a itemprop="url" href="'.Tools::safeOutput($category_link).'">'.
		'<span itemprop="title">'.
		htmlentities($end, ENT_NOQUOTES, 'UTF-8').
		'</span></a></div>';
		
		if ($id_category == $default_category)
			return $full_path;

		return Tools::getPath($id_category, $category->name, true, $type_cat).'<span class="navigation-pipe">'.$pipe.'</span> <span class="navigation_product">'.
		$full_path
		.'</span>';
	}
