...
<h1 class="page-heading">{l s='Authentication'}</h1>
<div class="retrieve-password">
	<p class="contact-intro">{l s='Please enter the email address you used to register. We will then send you a new password. '}</p>

	<div class="box">
		<h2 class="page-subheading">{l s='Forgot your password?'}</h2>
		...
		<form action="{$request_uri|escape:'html':'UTF-8'}" method="post" class="std" id="form_forgotpassword">
			<fieldset>
				...
				<p class="submit">
					<button type="submit" class="btn-blf white"><span>{l s='Retrieve Password'}</span></button>
				</p>
			</fieldset>
		</form>
		{/if}
...
<ul class="clearfix footer_links">
	<li><a class="btn-blf back" href="{$link->getPageLink('authentication')|escape:'html':'UTF-8'}" title="{l s='Back to Login'}" rel="nofollow"><span><i class="icon-chevron-left"></i> {l s='Back to Login'}</span></a></li>
</ul>
