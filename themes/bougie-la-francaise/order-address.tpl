	...
	{include file="$tpl_dir./errors.tpl"}
		...
		<form action="{$link->getPageLink($back_order_page, true)|escape:'html':'UTF-8'}" method="post">
			<p class="cart_navigation clearfix">
				<a href="{$link->getPageLink($back_order_page, true, NULL, "{if $back}back={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="btn-blf-cart-back">
...
<div class="addresses clearfix">
	<div class="row">
		...
		<div class="col-xs-12 col-sm-6">
			<div id="address_invoice_form" class="select form-group selector1"{if $cart->id_address_invoice == $cart->id_address_delivery} style="display: none;"{/if}>
				...
				{else}
					<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Add'}" class="btn-blf-cart-back">
	...
	<p class="address_add submit">
		<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Add'}" class="btn-blf-cart-back">
			<span>{l s='Add a new address'}</span>
			<i class="ycon-right-open-big"></i>
		</a>
	</p>
...
{if !$opc}
			<p class="cart_navigation clearfix">
				...
				<a href="{$link->getPageLink($back_order_page, true, NULL, "{if $back}back={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="btn-blf-cart-back">
					<i class="ycon-left-open-big"></i><span>{l s='Back'}</span>
				</a>
				<button type="submit" name="processAddress" class="btn-blf-cart-next">
					<span>{l s='Proceed to checkout'}</span>
				</button>
