					...
					{foreach from=$orders item=order name=myLoop}
						<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
							...
							<td class="history_detail">
								<a class="btn-blf btn-blf-smaller" href="javascript:showOrder(1, {$order.id_order|intval}, '{$link->getPageLink('order-detail', true, NULL, "id_order={$order.id_order|intval}")|escape:'html':'UTF-8'}');">
									<span>
										{l s='Details'}<i class="ycon-right-open-big"></i>
									</span>
								</a>
								{if isset($opc) && $opc}
...
<ul class="footer_links clearfix">
	<li>
		<a class="btn-blf back" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}">
			<span>
				<i class="ycon-left-open-big"></i> {l s='Back to Your Account'}
			</span>
		</a>
	</li>
	<li>
		<a class="btn-blf back" href="{$base_dir}">
			<span><i class="ycon-left-open-big"></i> {l s='Home'}</span>
		</a>
	</li>
</ul>
