		...
		<div class="row">
			<div class="alert alert-danger">
				...
				{if isset($smarty.server.HTTP_REFERER) && !strstr($request_uri, 'authentication') && preg_replace('#^https?://[^/]+/#', '/', $smarty.server.HTTP_REFERER) != $request_uri}
					<p class="lnk"><a class="alert-link" href="{$smarty.server.HTTP_REFERER|escape:'html':'UTF-8'|secureReferrer}" title="{l s='Back'}"><i class="icon-chevron-left"></i> {l s='Back'}</a></p>
				{/if}
			</div>
		</div>
	</div>
