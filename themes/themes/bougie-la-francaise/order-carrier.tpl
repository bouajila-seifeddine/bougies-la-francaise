	...
	<div id="carrier_area">
		...
		<form id="form" action="{$link->getPageLink('order', true, NULL, "{if $multi_shipping}multi-shipping={$multi_shipping}{/if}")|escape:'html':'UTF-8'}" method="post" name="carrier_area">
			<p class="cart_navigation clearfix">
				{if !$is_guest}
					{if $back}
						<a href="{$link->getPageLink('order', true, NULL, "step=1&back={$back}{if $multi_shipping}&multi-shipping={$multi_shipping}{/if}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="btn-blf-cart-back">
							<i class="ycon-left-open-big"></i>
							<span>{l s='Back'}</span>
						</a>
					{else}
						<a href="{$link->getPageLink('order', true, NULL, "step=1{if $multi_shipping}&multi-shipping={$multi_shipping}{/if}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="btn-blf-cart-back">
							<i class="ycon-left-open-big"></i>
							<span>{l s='Back'}</span>
						</a>
					{/if}
				{else}
					<a href="{$link->getPageLink('order', true, NULL, "{if $multi_shipping}multi-shipping={$multi_shipping}{/if}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="btn-blf-cart-back">
						<i class="ycon-left-open-big"></i>
						<span>{l s='Back'}</span>
					</a>
				{/if}
