...
{else}
	...
	<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" class="std" enctype="multipart/form-data">
		<fieldset>
			...
			{else}
				<select required="required" id="id_contact" name="id_contact" onchange="showElemFromSelect('id_contact', 'desc_contact')">
			...
			<p class="text">
				...
				{if isset($customerThread.email)}
					<input type="email" required="required" id="email" name="from" value="{$customerThread.email|escape:'htmlall':'UTF-8'}" readonly="readonly" />
				{else}
					<input type="email" required="required" id="email" name="from" value="{$email|escape:'htmlall':'UTF-8'}" />
				{/if}
			</p>
		{if !$PS_CATALOG_MODE}
			...
			<p class="text select product-choice">
		...
		<p class="textarea">
            <label for="message">{l s='Message'} <sup>*</sup></label>
			 <textarea required="required" id="message" name="message" rows="15" cols="10">{if isset($message)}{$message|escape:'htmlall':'UTF-8'|stripslashes}{/if}</textarea>
		</p>
		<p class="submit">
			<input required="required" type="submit" name="submitMessage" id="submitMessage" value="{l s='Send'}" class="button_large" />
		</p>
	</fieldset>
...
<div id="right-contact">
    <div class="right-contact-inner">
        <h3>{l s='Customer Service'}</h3>
        <p id="blf">{l s='Bougies la Française'}</p>
        <p id="address">{l s='Rue du président Auguste Durand – BP 70002- 85610 CUGAND- France'}</p>
        <p id="tel">{l s='Customer service at your disposal to '} <span>+33 2 51 42 19 72</span></p>
        <p id="days">{l s='Mondays-Friday - 8h-19h'}</p>
    </div>
    
    <div class="right-contact-inner">
        <h3>{l s='Frequently Asked Questions'}</h3>
        <a id="faq-link" href="{$link->getCMSLink(12)|escape:'html'}">{l s='See'}</a>
    </div>
</div>
{/if}
