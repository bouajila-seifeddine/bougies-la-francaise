	...
	<tr>
		<td style="width: 100%">
			{if !empty($delivery_address)}
				<table style="width: 100%">
					<tr>
                        <td style="width: 40%"><!-- CUSTOMER INFORMATION --><b>{l s='Order Number:' pdf='true'}</b> {$order->getUniqReference()}<br />
			...
			{else}
				<table style="width: 100%">
					<tr>
						...
						<td style="width: 60%"><!-- CUSTOMER INFORMATION --><b>{l s='Order Number:' pdf='true'}</b> {$order->getUniqReference()}<br />
                            <b>{l s='Order Date:' pdf='true'}</b> {dateFormat date=$order->date_add full=0}<br />
                            <b>{l s='Payment Method:' pdf='true'}</b>
                            {foreach from=$order_invoice->getOrderPaymentCollection() item=payment}
                                {$payment->payment_method}
						...
						<td style="width: 40%"><span style="font-weight: bold; font-size: 10pt; color: #9E9F9E">{l s='Billing & Delivery Address.' pdf='true'}</span><br />
							 {$invoice_address}
						</td>
					</tr>
				</table>
		...
		<td style="width: 100%; text-align: right">
			<table style="width: 100%; font-size: 8pt;">
				<tr style="line-height:4px;">
					<td style="text-align: left; background-color: #4D4D4D; color: #FFF; padding-left: 10px; line-height:7px; font-weight: bold; width: {if !$tax_excluded_display}45%{else}55%{/if}">{l s='Product / Reference' pdf='true'}</td>
                    <td style="text-align: left; background-color: #4D4D4D; color: #FFF; padding-left: 10px; line-height:7px; font-weight: bold; width: 10%">{l s='Reference' pdf='true'}</td>
                    <!-- unit price tax excluded is mandatory -->
					{*if !$tax_excluded_display}
						<td style="background-color: #4D4D4D; color: #FFF; text-align: right; font-weight: bold; width: 0%">{l s='Unit Price' pdf='true'} <br />{l s='(Tax Excl.)' pdf='true'}</td>
					{/if*}
					<td style="background-color: #4D4D4D; color: #FFF; text-align: right; line-height:7px; font-weight: bold; width: 15%">
					...
					<td style="background-color: #4D4D4D; color: #FFF; text-align: right; line-height:7px; font-weight: bold; width: 10%; white-space: nowrap;">{l s='Discount' pdf='true'}</td>
					<td style="background-color: #4D4D4D; color: #FFF; text-align: center; line-height:7px; font-weight: bold; width: 10%">{l s='Qty' pdf='true'}</td>
					<td style="background-color: #4D4D4D; color: #FFF; text-align: right; line-height:7px; font-weight: bold; width: {if !$tax_excluded_display}15%{else}25%{/if}">
						{l s='Total' pdf='true'}
						{if $tax_excluded_display}
							{l s='(Tax Excl.)' pdf='true'}
						{else}
