...
// <![CDATA[
	...
	function buildAddressBlock(id_address, address_type, dest_comp)
	{
		...
		li_content['update'] = '<a href="{$link->getPageLink('address', true, NULL, "id_address")|addslashes}'+id_address+'&amp;back=order?step=1{if $back}&mod={$back}{/if}">{l s='Update' js=1}</a>';
...
{/if}
	<div class="addresses clearfix">
		...
		{else}
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{'&multi-shipping=1'|urlencode}{if $back}&mod={$back}{/if}")|escape:'html'}" class="button_large">{l s='Add a new address'}</a>
		{/if}
		</p>
		<div class="clearfix">
		...
		<p class="address_add submit">
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{'&multi-shipping=1'|urlencode}{if $back}&mod={$back|urlencode}{/if}")|escape:'html'}" class="button_large">&raquo; {l s='Add a new address'}</a>
		</p>
		{if !$opc}
...
{if !$opc}
	<p class="cart_navigation submit">
		...
		{if $back}
			<a href="{$link->getPageLink('order', true, NULL, "step=0&amp;back={$back}")|escape:'html'}" class="button">&laquo; {l s='Previous'}</a>
		{else}
			<a href="{$link->getPageLink('order', true, NULL, "step=0")|escape:'html'}" class="button">&laquo; {l s='Previous'}</a>
		{/if}
