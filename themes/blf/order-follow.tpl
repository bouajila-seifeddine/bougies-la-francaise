...
{if isset($errorMsg) && $errorMsg}
	<div class="error">
		<p>{l s='Please provide an explanation for your RMA.'}</p>
	</div>
	<p>
...
<p class="intro">{l s='Here is a list of pending merchandise returns'}.</p>
	...
	<table id="order-list" class="std">
		...
		{foreach from=$ordersReturn item=return name=myLoop}
			<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
				...
				{if $return.state == 2}
					<a href="{$link->getPageLink('pdf-order-return', true, NULL, "id_order_return={$return.id_order_return|intval}")|escape:'html'}"><img src="{$img_dir}icon/pdf.gif" alt="{l s='Order return'} {l s='#'}{$return.id_order_return|string_format:"%06d"}" class="icon" /></a>
					<a href="{$link->getPageLink('pdf-order-return', true, NULL, "id_order_return={$return.id_order_return|intval}")|escape:'html'}">{l s='Print out'}</a>
				{else}
					--
				{/if}
				</td>
