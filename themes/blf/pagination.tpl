...
{if isset($p) AND $p}
	...
	{if $start!=$stop}
		...
		{if $p != 1}
			...
			<li class="pagination_previous"><a {$no_follow_text} href="{$link->goPage($requestPage, $p_previous)}">{l s='Previous'}</a></li>
		{/if}
		{if $start==3}
		...
		{if $pages_nb > 1 AND $p != $pages_nb}
			...
			<li class="pagination_next"><a {$no_follow_text} href="{$link->goPage($requestPage, $p_next)}">{l s='Next'}</a></li>
		{/if}
		</ul>
	{/if}
	<!-- /Pagination -->
