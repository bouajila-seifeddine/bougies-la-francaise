...
// <![CDATA[
	...
	function buildAddressBlock(id_address, address_type, dest_comp)
	{
		...
		li_content['update'] = '<a href="{$link->getPageLink('address', true, NULL, "id_address")|addslashes}'+id_address+'&amp;back={$back_order_page}?step=1{if $back}&mod={$back}{/if}">&raquo; {l s='Update' js=1}</a>';
...
{if !$opc}
	<p class="cart_navigation submit">
		...
		<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")|escape:'html'}" class="button prev">{l s='Previous'}</a>
		<input type="submit" name="processAddress" value="{l s='Next'}" class="exclusive next" />
	</p>
</form>
{else}
</div>
