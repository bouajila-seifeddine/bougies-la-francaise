...
{if isset($cms) && !isset($cms_category)}
	...
	{/if}
	<div id="cms-content" class="rte{if $content_only} content_only{/if} {$parent_categorie->name|lower}">
		{$cms->content}
        
        {if $cms->id == 19}
            {* Display hook footer here for newsletter -> they move this block from footer to this page *}
            {$HOOK_FOOTER}
        {/if}
	</div>
