		...
		<ul class="products clearfix">
			{foreach from=$productsViewedObj item=viewedProduct name=myLoop}
				<li class="clearfix{if $smarty.foreach.myLoop.last} last_item{elseif $smarty.foreach.myLoop.first} first_item{else} item{/if}">
					<a href="{$viewedProduct->product_link|escape:'html'}" class="content_img">
					...
					<div class="text_desc">
						<p class="s_title_block"><a href="{$viewedProduct->product_link|escape:'html'}">{$viewedProduct->name|truncate:14:'...'|escape:html:'UTF-8'}</a></p>
						<p><a href="{$viewedProduct->product_link|escape:'html'}">{$viewedProduct->description_short|strip_tags:'UTF-8'|truncate:44}</a></p>
					</div>
				</li>
			{/foreach}
		</ul>
