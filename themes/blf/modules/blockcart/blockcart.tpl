...
<div id="cart_block">
	...
	<p class="title_block">
		<a href="{$link->getPageLink("$order_process", true)|escape:'html'}" rel="nofollow">{l s='Cart' mod='blockcart'}
		{if $ajax_allowed}
	...
	{if $products}
		...
		{foreach from=$products item='product' name='myLoop'}
			...
			<dd id="cart_block_combination_of_{$product.id_product}{if $product.id_product_attribute}_{$product.id_product_attribute}{/if}_{$product.id_address_delivery|intval}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
				<a href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'html'}">{$product.attributes_small}</a>
			{/if}
			<!-- Customizable datas -->
	...
	{/if}
			...
			{foreach from=$discounts item=discount}
				{if $discount.value_real > 0}
					<tr class="bloc_cart_voucher" id="bloc_cart_voucher_{$discount.id_discount}">
						...
						<td class="name">{$discount.name|cat:' : '|cat:$discount.description|truncate:18:'...'|escape:'htmlall':'UTF-8'}</td>
						...
						<td class="delete">
							{if strlen($discount.code)}
								<a class="delete_voucher" href="{$link->getPageLink('$order_process', true)}?deleteDiscount={$discount.id_discount}" rel="nofollow"></a>
							{/if}
						</td>
					</tr>
				{/if}
		...
		{/if}
		<p id="cart-buttons">
			{if $order_process == 'order'}<a href="{$link->getPageLink("$order_process", true)|escape:'html'}" class="button_small" rel="nofollow">{l s='Cart' mod='blockcart'}</a>{/if}
			<a href="{$link->getPageLink("$order_process", true)|escape:'html'}" id="button_order_cart" class="exclusive{if $order_process == 'order-opc'}_large{/if}" rel="nofollow"><span></span>{l s='Check out' mod='blockcart'}</a>
		</p>
	</div>
	</div>
</div>
