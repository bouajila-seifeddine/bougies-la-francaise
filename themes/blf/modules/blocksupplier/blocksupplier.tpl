...
<div id="suppliers_block_left" class="block blocksupplier">
	<p class="title_block">{if $display_link_supplier}<a href="{$link->getPageLink('supplier')|escape:'html'}">{/if}{l s='Suppliers' mod='blocksupplier'}{if $display_link_supplier}</a>{/if}</p>
...
{if $suppliers}
	...
	{foreach from=$suppliers item=supplier name=supplier_list}
		...
		<li class="{if $smarty.foreach.supplier_list.last}last_item{elseif $smarty.foreach.supplier_list.first}first_item{else}item{/if}">
			<a href="{$link->getsupplierLink($supplier.id_supplier, $supplier.link_rewrite)|escape:'html'}">{$supplier.name|escape:'htmlall':'UTF-8'}</a>
		</li>
		{/if}
	{/foreach}
	</ul>
