	...
	<li class="item" id="cb-{$product.id_order_detail|intval}" data-icon="back">
		...
		<h3>
			{if $product.download_hash && $invoice && $product.display_filename != ''}
				...
				<a href="{$link->getPageLink('get-file', true, NULL, "key={$product.filename|escape:'htmlall':'UTF-8'}-{$product.download_hash|escape:'htmlall':'UTF-8'}&amp;id_order={$order->id}&secure_key={$order->secure_key}")|escape:'html'}" data-ajax="false">
				{else}
					<a href="{$link->getPageLink('get-file', true, NULL, "key={$product.filename|escape:'htmlall':'UTF-8'}-{$product.download_hash|escape:'htmlall':'UTF-8'}")|escape:'html'}" data-ajax="false">
				{/if}
