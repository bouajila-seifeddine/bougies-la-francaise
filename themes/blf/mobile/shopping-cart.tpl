	...
	<div data-role="content" id="content" class="cart">
		...
		<ul data-role="listview" data-inset="true" data-split-theme="d" data-split-icon="delete">
			...
			<li>
				...
				{if strlen($discount.code)}<a href="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}?deleteDiscount={$discount.id_discount}" class="price_discount_delete" data-ajax="false">{l s='Delete'}</a>{/if}
			</li>
			{/foreach}
		</ul>
		{/if}
