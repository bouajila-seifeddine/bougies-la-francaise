...
// <![CDATA[
	...
	function buildAddressBlock(id_address, address_type, dest_comp)
	{
		...
		li_content['update'] = '<a href="{$link->getPageLink('address', true, NULL, "id_address")|addslashes}'+id_address+'&amp;back={$back_order_page}?step=1{if $back}&mod={$back}{/if}" style="font-size:.9em;font-weight:normal;padding:8px 0;display:block;text-align:right">&raquo; {l s='Update' js=1}</a>';
	...
	<div data-role="content" id="content" class="cart">
				...
				<div class="address-form-multishipping">
					<div class="button_multishipping_mode" id="multishipping_mode_box">
						...
						<div class="description_off">
							<a href="{$link->getPageLink('order-opc', true, NULL, 'ajax=1&multi-shipping=1&method=multishipping')|escape:'html'}" id="link_multishipping_form">
		...
		{/if}
			<div class="addresses clearfix">
				...
				{if $addresses|@count > 1}
					...
					{else}
						<a style="margin-left: 221px;" href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")|escape:'html'}" class="button_large">{l s='Add a new address'}</a>
					{/if}
				</p>
			</div>
			<div class="addresses">
				...
				<p class="address_add submit">
					<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")|escape:'html'}" class="button_large" data-role="button" data-icon="plus">{l s='Add a new address'}</a>
				</p>
				{if !$opc}
		...
		{if !$opc}
			<fieldset class="cart_navigation submit ui-grid-a">
				...
				<div class="ui-block-a"><a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")|escape:'html'}" data-role="button" data-icon="back" data-ajax="false">{l s='Previous'}</a></div>
