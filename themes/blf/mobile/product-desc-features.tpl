			...
			<ul>
				{foreach from=$accessories item=accessory name=accessories_list}
					...
					<li class="ajax_block_product {if $smarty.foreach.accessories_list.last}last_item{else}item{/if} product_accessories_description clearfix">
						...
						{/if}
						<div class="btn-row">
							<a class="" data-theme="a" data-role="button" data-mini="true" data-inline="true" data-icon="arrow-r" href="{$accessoryLink|escape:'htmlall':'UTF-8'}" data-ajax="false">{l s='View'}</a>
							...
							{/if}
							<a class="{$btn_class}" data-role="button" data-inline="true" data-theme="e" data-icon="plus" data-mini="true" class="exclusive button ajax_add_to_cart_button" href="{$btn_href}" rel="ajax_id_product_{$accessory.id_product|intval}" data-ajax="false">{l s='Add to cart'}</a>
							{$btn_more}
						</div><!-- .btn-row -->
					</li>
				{/foreach}
