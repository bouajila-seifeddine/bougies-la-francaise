...
class Template extends ObjectModel{
	...
	private function editContent(&$content, $save = true){
		...
		$logo = $context->shop->getBaseUrl().'img/'.Configuration::get('PS_LOGO_MAIL');
	...
	public function editTemplate($content, $id_cart = NULL, $id_lang = null){
		...
		$query = '
			...
			WHERE 
			pl.id_lang = c.id_lang AND (pl.id_lang = ' . (int)$id_lang . ' OR pl.id_lang = ' . $lang_default . ')';
		if(is_null($id_cart))
			$query .= ' LIMIT 1';
		...
		foreach($products as $product){
			...
			$html .= '<tr>';
			$html .= '<td width="1">';
			$html .= '<a target="_blank" style="text-decoration: none;" href="' . $link->getProductLink($product['id_product']) . '"><img width="75" valign="bottom" src="http://' . $img . '"></a>';
			$html .= '</td>';
			$html .= '<td align="left" valign="middle" style="font-size: 12px">&nbsp;&nbsp;&nbsp;&nbsp;<a style="text-decoration: none;" target="_blank" href="' . $link->getProductLink($product['id_product']) . '">' . $product['name'] . '</a></td>';
			$html .= '</tr>';
	...
	public static function editBeforeSending($content, $id_cart = NULL, $id_lang = 1, $wichRemind){
		...
		$products = Db::getInstance()->ExecuteS('
				...
				WHERE 
				ca.id_cart = ' . (int)$id_cart . '
				AND pl.id_lang = c.id_lang AND (pl.id_lang = ' . (int)$id_lang . ' OR pl.id_lang = ' . $lang_default . ')');
		...
		if(strpos($content,'%CART_PRODUCTS%')){
			...
			foreach($products as $product){
				...
				$html .= '<td width="1">';
				$html .= '<a width="1" target="_blank" style="text-decoration: none;" href="' . $link->getProductLink($product['id_product']) . '"><img width="75" valign="bottom" src="http://' . $img . '"></a>';
				$html .= '</td>';
				$html .= '<td align="left" valign="middle" style="font-size: 11px; padding-left: 20px"><a style="text-decoration: none;" target="_blank" href="' . $link->getProductLink($product['id_product']) . '">' . $product['name'] . '</a></td>';
				$html .= '</tr>';
				// $product .= str_replace('%IMG%', '<img src="http://' . $img . '">', $cartProducts);
				// $product .= str_replace('%NAME%', Product::getProductName($product['id_product'], null, $product['id_lang']), $product);
				// $html .= $product;
