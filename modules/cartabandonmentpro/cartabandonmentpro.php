...
class CartAbandonmentPro extends Module
{
	...
	public function __construct()
	{
		...
		$this->ps_version_compliancy 	= array('min' => '1.5.0.0', 'max' => '1.6.9');
	...
	*/
	...
	private function handleDiscounts()
	{
		...
		{
			if(Tools::getValue('discounts_active'))
			{
				if(Tools::getValue('discounts_different_val') == 0)
				{
					$query = 'DELETE FROM '._DB_PREFIX_."cartabandonmentpro_cartrule;";
					Db::getInstance()->Execute($query);
					$query = 'INSERT INTO '._DB_PREFIX_.'cartabandonmentpro_cartrule VALUE (NULL, "'.Tools::getValue('discounts_type').'", "'.Tools::getValue('discounts_value').'", NULL, NULL, "'.Tools::getValue('discounts_validity').'", "'.Tools::getValue('discounts_validity_days').'", NULL, '.Tools::getValue('discounts_min').', '.Tools::getValue('discounts_max').', '.Tools::getValue('discount_template').', 0);';
					Db::getInstance()->Execute($query);
				}
				else
				{
					...
					for($x = 1; $x <= 3; $x++)
					{
						$query = 'INSERT INTO '._DB_PREFIX_.'cartabandonmentpro_cartrule VALUE (NULL, "'.Tools::getValue('discounts_type_'.$x).'", "'.Tools::getValue('discounts_value_'.$x).'", NULL, NULL, "time", "'.Tools::getValue('discounts_validity_days_'.$x).'", NULL, '.Tools::getValue('discounts_min_'.$x).', '.Tools::getValue('discounts_max_'.$x).', '.Tools::getValue('discount_template').', '.$x.');';
						Db::getInstance()->Execute($query);
						if($tranches == $x)
							break;
					}
		...
		if(Tools::getIsset('activateCronJob'))
		{
			...
			if(Tools::getValue('activateCronJob') == 1)
			{
				...
				if($activated[0]['activated'] == 0)
				{
					...
					Db::getInstance()->Execute("INSERT INTO "._DB_PREFIX_."cronjobs VALUES (NULL, NULL, 'Automatic cronjob cartabandonment pro 1', '".$uri."modules/cartabandonmentpro/send.php?id_shop=".$this->context->shop->id."&token=".Configuration::get('CARTABAND_TOKEN')."&wich_remind=1', '-1', '-1', '-1', '-1', NULL, 0, 1, ".$this->context->shop->id.", ".$this->context->shop->id_shop_group.")");
					
					Db::getInstance()->Execute("INSERT INTO "._DB_PREFIX_."cronjobs VALUES (NULL, NULL, 'Automatic cronjob cartabandonment pro 2', '".$uri."modules/cartabandonmentpro/send.php?id_shop=".$this->context->shop->id."&token=".Configuration::get('CARTABAND_TOKEN')."&wich_remind=2', '-1', '-1', '-1', '-1', NULL, 0, 1, ".$this->context->shop->id.", ".$this->context->shop->id_shop_group.")");

					Db::getInstance()->Execute("INSERT INTO "._DB_PREFIX_."cronjobs VALUES (NULL, NULL, 'Automatic cronjob cartabandonment pro 3', '".$uri."modules/cartabandonmentpro/send.php?id_shop=".$this->context->shop->id."&token=".Configuration::get('CARTABAND_TOKEN')."&wich_remind=3', '-1', '-1', '-1', '-1', NULL, 0, 1, ".$this->context->shop->id.", ".$this->context->shop->id_shop_group.")");
	...
	private function initVars(){
		...
		$this->context->smarty->assign('discount_val_text', Configuration::get('CARTABAND_DISC_VAL', $id_lang));
		$this->context->smarty->assign('discount_shipping_text', Configuration::get('CARTABAND_SHIPP_VAL', $id_lang));
		...
		if (Module::isInstalled('cronjobs'))
			$activated = Db::getInstance()->ExecuteS("SELECT COUNT(*) as activated FROM "._DB_PREFIX_."cronjobs WHERE description = 'Automatic cronjob cartabandonment pro 1'");
