...
if(!@class_exists('SystempayApi', false)) {
	...
	class SystempayApi {
		...
		*/
		...
		/**
		 * Returns an array of languages accepted by the Systempay payment page
		 * @static
...
if(!@class_exists('SystempayResponse', false)) {
	...
	class SystempayResponse {
		 ...
		 */
		function SystempayResponse($parameters = null, $ctx_mode = null, $key_test = null, $key_prod = null) {
			$this->api = new SystempayApi(); // Use default API encoding (UTF-8) since the payment platform returns UTF-8 data 
...
if(!@class_exists('SystempayMultiApi', false)) {
	/**
	 * Class managing multi payment specific actions
	 * @version 2.2
	 */
	class SystempayMultiApi extends SystempayApi {
	
		/**
		 * Constructor.
		 */
		function SystempayMultiApi($encoding="UTF-8") {
			// call parent class constructor
			parent::SystempayApi($encoding);
		}
