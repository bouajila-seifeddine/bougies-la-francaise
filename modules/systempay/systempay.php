...
require_once dirname(__FILE__) . '/classes/systempay_api.php';
...
class Systempay extends PaymentModule {
	...
	private $_multiLangFields = array('SYSTEMPAY_STD_TITLE', 'SYSTEMPAY_MULTI_TITLE', 'SYSTEMPAY_ONEY_TITLE');
	private $_amountFields = array('SYSTEMPAY_3DS_MIN_AMOUNT', 'SYSTEMPAY_STD_AMOUNT_MIN', 'SYSTEMPAY_STD_AMOUNT_MAX',
			'SYSTEMPAY_MULTI_AMOUNT_MIN', 'SYSTEMPAY_MULTI_AMOUNT_MAX'
	);
	private $_systempayApi = null;
	public $logger = null;
	...
	public function __construct() {
		...
		$this->description = sprintf($this->l('Accept payments by credit cards with %s'), ' Systempay');
	...
	private function _getAdminParameters() {
		...
		return array(
				...
				array('key' => 'SYSTEMPAY_KEY_TEST', 'name' => 'key_test', 'default' => '1111111111111111', 'label' => 'Test certificate'),
				array('key' => 'SYSTEMPAY_KEY_PROD', 'name' => 'key_prod', 'default' => '2222222222222222', 'label' => 'Production certificate'),
				array('key' => 'SYSTEMPAY_MODE', 'name' => 'ctx_mode', 'default' => 'TEST', 'label' => 'Mode'),
				array('key' => 'SYSTEMPAY_PLATFORM_URL', 'name' => 'platform_url', 'default' => 'https://paiement.systempay.fr/vads-payment/', 'label' => 'Gateway URL'),
				...
				array('key' => 'SYSTEMPAY_DELAY', 'name' => 'capture_delay', 'default' => '', 'label' => 'Delay'),
				array('key' => 'SYSTEMPAY_VALIDATION_MODE', 'name' => 'validation_mode', 'default' => '', 'label' => 'Payment validation'),
				array('key' => 'SYSTEMPAY_PAYMENT_CARDS', 'name' => 'payment_cards', 'default' => 'CB;VISA;VISA_ELECTRON;MASTERCARD;MAESTRO;E-CARTEBLEUE', 'label' => 'Available payment cards'),
				...
				array('key' => 'SYSTEMPAY_3DS_MIN_AMOUNT', 'default' => '', 'label' => 'Minimum amount for which activate 3DS'),
				
				array('key' => 'SYSTEMPAY_REDIRECT_ENABLED', 'name' => 'redirect_enabled', 'default' => 'False', 'label' => 'Automatic redirection'),
				array('key' => 'SYSTEMPAY_REDIRECT_SUCCESS_T', 'name' => 'redirect_success_timeout', 'default' => '5', 'label' => 'Success timeout'),
				array('key' => 'SYSTEMPAY_REDIRECT_SUCCESS_M', 'name' => 'redirect_success_message', 'default' => 'Redirection vers la boutique dans quelques instants...', 'label' => 'Success message'),
				array('key' => 'SYSTEMPAY_REDIRECT_ERROR_T', 'name' => 'redirect_error_timeout', 'default' => '5', 'label' => 'Failure timeout'),
				array('key' => 'SYSTEMPAY_REDIRECT_ERROR_M', 'name' => 'redirect_error_message', 'default' => 'Redirection vers la boutique dans quelques instants...', 'label' => 'Failure message'),
				...
				array('key' => 'SYSTEMPAY_RETURN_GET_PARAMS', 'name' => 'return_get_params', 'default' => '', 'label' => 'Additional GET parameters'),
				array('key' => 'SYSTEMPAY_RETURN_POST_PARAMS', 'name' => 'return_post_params', 'default' => '', 'label' => 'Additional POST parameters'),
				
				array('key' => 'SYSTEMPAY_STD_TITLE', 'default' => array('en' => 'Payment by bank card', 'fr' => 'Paiement par carte bancaire'), 'label' => 'Method title'),
				...
				array('key' => 'SYSTEMPAY_MULTI_TITLE', 'default' => array('en' => 'Payment by bank card in several times', 'fr' => 'Paiement par carte bancaire en plusieurs fois'), 'label' => 'Method title'),
				...
				array('key' => 'SYSTEMPAY_ONEY_TITLE', 'default' => array('en' => 'Payment with FacilyPay Oney', 'fr' => 'Paiement avec FacilyPay Oney'), 'label' => 'Method title'),
				...
				array('key' => 'SYSTEMPAY_ONEY_PROD_CAT', 'default' => array(), 'label' => 'Product categories options'),
				array('key' => 'SYSTEMPAY_ONEY_CAT_MODE', 'default' => '0', 'label' => ''),
				array('key' => 'SYSTEMPAY_ONEY_COM_CAT', 'default' => 'FOOD_AND_GROCERY', 'label' => '')
		);
	}

	/**
	 * Returns a new SystempayMultiApi object loaded with the module configuration
	 * @return SystempayMultiApi
	 */
	public function getLoadedApi() {
		if($this->_systempayApi == null) {
			$this->_systempayApi = new SystempayMultiApi();
			$this->_systempayApi->set('version', 'V2');
			$this->_systempayApi->set('contrib', 'Prestashop1.5.0.x_1.2f/' . _PS_VERSION_);
			
			foreach ($this->_getAdminParameters() as $param) {
				if(key_exists('name', $param) && isset($param['name'])) {
					// only set systempay payment params
					$this->_systempayApi->set($param['name'], Configuration::get($param['key']));
				}
			}
		}
		
		return $this->_systempayApi;
	}
	...
	public function uninstall() {
		...
		$result &= Db::getInstance()->execute("DELETE FROM `"._DB_PREFIX_."configuration` WHERE `name` LIKE 'SYSTEMPAY_%' AND `name` NOT LIKE 'SYSTEMPAY_OS_%'");
	...
	private function _postProcess() {
		$api = new SystempayApi(); // new instance of SystempayApi for parameters validation
		...
		foreach ($this->_getAdminParameters() as $param) {
			...
			if(in_array($key, $this->_multiLangFields)) {
			...
			} elseif($key === 'SYSTEMPAY_AVAILABLE_LANGUAGES' || $key === 'SYSTEMPAY_PAYMENT_CARDS') {
				$value = (is_array($value) && count($value) > 0) ? implode(';', $value) : '';
				
				if($value == '' && $key === 'SYSTEMPAY_PAYMENT_CARDS') {
					// empty values for SYSTEMPAY_PAYMENT_CARDS are not allowed
				...
				$value = serialize($value);
			} elseif($key === 'SYSTEMPAY_ONEY_PROD_CAT') {
				if (!is_array($value) || empty($value)) {
					$value = array();
				}
				$value = serialize($value);
			...
			} elseif ($key === 'SYSTEMPAY_STD_CARD_DATA_MODE' && $value == '3' && !$this->_checkSsl()) {
				$value = '1';
	...
	private function _validateOney() {
		...
		$amountMin = Tools::getValue('SYSTEMPAY_ONEY_AMOUNT_MIN', null);
		$amountMax = Tools::getValue('SYSTEMPAY_ONEY_AMOUNT_MAX', null);
	...
	private function _renderForm() {
		...
		return $this->context->smarty->fetch(_PS_MODULE_DIR_ . 'systempay/views/templates/back/back_office.tpl');
	}
	...
	public function hookPayment($params) {
		...
		if ($this->_checkStandardPayment()) {
 			$this->context->smarty->assign('systempay_std_title', Configuration::get('SYSTEMPAY_STD_TITLE', $this->context->language->id));
			...
			$this->context->smarty->assign('systempay_avail_cards', $cards);
			
			$entryMode = Configuration::get('SYSTEMPAY_STD_CARD_DATA_MODE');
			if($entryMode === '3' && !$this->_checkSsl()) { // no data entry on merchant site without SSL
				$entryMode = '1'; 
			}
			$this->context->smarty->assign('systempay_std_card_data_mode', $entryMode);
		...
		if ($this->_checkMultiPayment()) {
 			$this->context->smarty->assign('systempay_multi_title', Configuration::get('SYSTEMPAY_MULTI_TITLE', $this->context->language->id));
			
			// multi payment options
			$this->context->smarty->assign('systempay_multi_options', $this->_getAvailableMultiPaymentOptions());
			$_html .= $this->display(__FILE__, 'payment_multi.tpl');
		}
		
		
		if ($this->_checkOneyPayment()) {
			$this->context->smarty->assign('systempay_oney_title', Configuration::get('SYSTEMPAY_ONEY_TITLE', $this->context->language->id));
			...
			if (!empty($errors)) {
				$this->context->smarty->assign('systempay_oney_errors', $errors);
	...
	*/
	public function getFormFields($type='standard', $data=array()) {
		...
		if (!$api->isSupportedLanguage($language)) {
			$language = Configuration::get('SYSTEMPAY_DEFAULT_LANGUAGE');
		}
		...
		switch ($type) {
			...
			case 'oney':
				...
				if (Configuration::get('SYSTEMPAY_ONEY_CAT_MODE') == '1') {
					$category = Configuration::get('SYSTEMPAY_ONEY_COM_CAT');
				} else {
	...
	*/
	public function saveOrder($cart, $orderStatus, $systempayResponse) {
		...
		$currencyId = Currency::getIdByIsoCodeNum((int)$systempayResponse->get('currency'));
		
		// 3ds extra message
		$msg3ds = "\n" . $this->l('3DS authentication : ');
		if ($systempayResponse->get('threeds_status') == 'Y') {
			$msg3ds .= $this->l('YES');
			$msg3ds .= "\n" . $this->l('3DS certificate : ') . $systempayResponse->get('threeds_cavv');
		} else {
			$msg3ds .= $this->l('NO');
		}
		...
		$this->savePayment($order, $systempayResponse);
	 ...
	 */
	public function setOrderState($order, $orderState, $systempayResponse) {
		$this->logger->logInfo("Payment status for cart {$order->id_cart} has changed. New order status is $orderState.");
		
		$msg = new Message();
		$msg->message = $systempayResponse->getLogString();
	 ...
	 */
	public function savePayment($order, $systempayResponse) {
		...
		$currency = $this->getLoadedApi()->findCurrencyByNumCode($systempayResponse->get('currency'));
		
		$paymentIds = array();
		if ($systempayResponse->get('card_brand') == 'MULTI') {
			$sequences = json_decode($systempayResponse->get('payment_seq'));
			...
			$amount = $currency->convertAmountToFloat($systempayResponse->get('amount'));
			
			if (!$order->addOrderPayment($amount, null, $systempayResponse->get('trans_id'), null, null, $invoice)) {
				throw new PrestaShopException('Can\'t save Order Payment');
			}
			...
			// set card info
			$pcc->card_number = $systempayResponse->get('card_number');
			$pcc->card_brand = $systempayResponse->get('card_brand');
			if ($systempayResponse->get('expiry_month') && $systempayResponse->get('expiry_year')) {
				$pcc->card_expiration = str_pad($systempayResponse->get('expiry_month'), 2, '0', STR_PAD_LEFT) . '/' . $systempayResponse->get('expiry_year');
			}
			$pcc->card_holder = NULL;
	...
	private function _isSuccessState($order) {
		return $order->getCurrentState() == Configuration::get('PS_OS_PAYMENT')
				...
				|| $order->getCurrentState() == Configuration::get('SYSTEMPAY_OS_ONEY_PENDING')
	...
	public function isOney($systempayResponse) {
		return $systempayResponse->get('card_brand') == 'ONEY' || $systempayResponse->get('card_brand') == 'ONEY_SANDBOX';	
	}
	
	public function isOneyPendingPayment($systempayResponse) {
		return $this->isOney($systempayResponse) && $systempayResponse->isPendingPayment();
	}
}