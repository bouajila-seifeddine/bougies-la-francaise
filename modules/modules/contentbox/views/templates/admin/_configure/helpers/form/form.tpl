    ...
    {if $input.type == 'topform'}
        <div id='topform-label' >
             <div class="col-md-3 row" style="background-color: transparent;">
                <div class="top-logo">
                    <img src="{$input.logoImg}" alt="contentBox" style="float:left;">
                </div>
                <div class="col-md-8 top-module-description">
                    <h1 class="top-module-title">{$input.moduleName}</h1>

                    <div class="top-module-sub-title">{$input.moduleDescription}</div>

                    <div class="top-module-my-name"><a href="http://contentbox.org/?v={$input.moduleVersion}">contentBox {$input.moduleVersion}</a></div>
            ...
            <div class="col-md-4">
                ...
                <select id="contentbox_shop_select" name="contentbox_shop_select">
                    {foreach $input.shops as $shop}
                        <option id="id_{$shop['id_shop']}" value="{$shop['id_shop']}"
            ...
            <div class="col-md-4">
                ...
                <select id="contentbox_language_select" name="contentbox_language_select">
                    {foreach $input.languages as $language}
                        <option id="id_{$language['id_lang']}" value="{$language['id_lang']}"
            ...
            {if !empty( $input.files ) }
                {foreach $input.files as $pos=>$file}
                    {if $file['name'] != 'index.php'}
                        <div class="fileContainer" style="" rel="content/{$file['name']}" data-filename="{$file['name']}" data-filepath="{$input.path}">
                            <div class="fileUrl">
                                <img src="{$input.path}img/url.png" alt="URL"/>
                            </div>
                            <div class="fileDelete" >
                                <img src="{$input.path}img/close.png" alt="DELETE"/>
                            </div>
                            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    {if in_array( strtolower( $file['extension'] ), $input.imagesExtensions ) }
                                    <td style="height:90px;" valign="middle">
                                        <img src="{$input.path}content/{$file['name']}" style="max-width:100px; max-height:100px" />
                                    </td>
                                    {else}
                                    <td style="height:90px;" valign="middle">
                                        *.{$file['extension']}
