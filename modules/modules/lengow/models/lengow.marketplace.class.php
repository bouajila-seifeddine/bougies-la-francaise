...
class LengowMarketplaceAbstract {
	...
	*/
	protected function _matchCarrier($param, $name)
	{
		// No match
		if (!isset($param['accepted_values']))
			return $name;
	...
	*/
	public static function getMarkeplaces()
	{
		$marketplaces = array();
		self::_loadXml();
		foreach (self::$DOM->marketplace as $marketplace)
		{
			$attributes = $marketplace->attributes();
			$marketplaces[(string)$attributes['name']] = (string)$attributes['name'];
		}
		ksort($marketplaces);
	...
	*/
	private static function _loadXml()
	{
		if (!self::$DOM)
		{
			if (_PS_MODULE_DIR_)
