...
//<![CDATA[
	...
	var url_allinone_sponsorship="{$link->getModuleLink('allinone_rewards', 'sponsorship', [], true)}";
//]]>
</script>
...
<div id="rewards_sponsorship" class="rewards">
	...
	<div class="sheets">
		...
		{if $canSendInvitations || isset($popup)}
			<div id="sponsorship_form"  {if isset($popup) && !$afterSubmit}style="display: none"{/if}>
				...
				<ul>
					<li>{l s='Propose your sponsorship on the social networks, by clicking the following links' mod='allinone_rewards'}<br>
						&nbsp;<a href="http://www.facebook.com/sharer.php?u={$link_sponsorship_fb}" target="_blank" title="{l s='Facebook' mod='allinone_rewards'}"><img src='{$rewards_path}images/facebook.png' height='20'></a>
						&nbsp;<a href="http://twitter.com/share?url={$link_sponsorship_twitter}" target="_blank" title="{l s='Twitter' mod='allinone_rewards'}"><img src='{$rewards_path}images/twitter.png' height='20'></a>
						&nbsp;<a href="https://plus.google.com/share?url={$link_sponsorship_google}" target="_blank" title="{l s='Google+' mod='allinone_rewards'}"><img src="{$rewards_path}images/google.png"></a>
					</li>
			{if $open_inviter_providers && count($open_inviter_providers)}
				...
				{foreach from=$open_inviter_providers key='provider' item='details'}
								<option value='{$provider}'>{$details.name}</option>
				{/foreach}
							</select>
						...
						<div id="open_inviter_contacts">
							<center><img src="{$base_dir_ssl}modules/allinone_rewards/images/loadingAnimation.gif"></center>
						</div>
					</li>
			{/if}
			...
			{if $sms}
					<li>
						<form id="sms_form" method="post" action="{$link->getModuleLink('allinone_rewards', 'sponsorship', [], true)}" style="display: inline">{l s='Enter their mobile phone (international format) and send them a SMS' mod='allinone_rewards'} <input id="phone" name="phone" maxlength="16" type="text" placeholder="{l s='e.g. +33612345678' mod='allinone_rewards'}" />
							<input type="image" src="{$base_dir_ssl}modules/allinone_rewards/images/sendsms.gif" id="submitSponsorSMS" name="submitSponsorSMS" alt="{l s='Send SMS' mod='allinone_rewards'}" title="{l s='Send SMS' mod='allinone_rewards'}" align="absmiddle" />
						</form>
					</li>
			{/if}
				...
				<div>
					<form id="list_contacts_form" method="post" action="{$link->getModuleLink('allinone_rewards', 'sponsorship', [], true)}">
						<table class="std">
						<thead>
							<tr>
						...
						<p class="checkbox">
							...
							<a href="{$link->getModuleLink('allinone_rewards', 'rules', ['sback' => $sback], true)}" class="fancybox rules" title="{l s='Conditions of the sponsorship program' mod='allinone_rewards'}">{l s='Read conditions' mod='allinone_rewards'}</a>
						</p>
						<p>
							{l s='Preview' mod='allinone_rewards'} <a href="{$link->getModuleLink('allinone_rewards', 'email', ['sback' => $sback], true)}" class="fancybox mail" title="{l s='Invitation email' mod='allinone_rewards'}">{l s='the default email' mod='allinone_rewards'}</a> {l s='that will be sent to your friends.' mod='allinone_rewards'}
						</p>
		...
		<div id="idTab2" class="sponsorshipBlock">
			...
			<div>
				<form method="post" action="{$link->getModuleLink('allinone_rewards', 'sponsorship', [], true)}" class="std">
					<table class="std">
					<thead>
						<tr>
							<th class="first_item">&nbsp;</th>
					...
					{foreach from=$pendingFriends item=pendingFriend name=myLoop}
						<tr class="{if ($smarty.foreach.myLoop.iteration % 2) == 0}item{else}alternate_item{/if}">
							<td>
								<input type="checkbox" name="friendChecked[{$pendingFriend.id_sponsorship}]" id="friendChecked[{$pendingFriend.id_sponsorship}]" value="1" />
							</td>
...
<p class="error">
		...
		{else}
			{l s='Error :' mod='allinone_rewards'} {$error}
		{/if}
</p>
	{else}
<form id="open_inviter_contacts_form" method="post" action="{$link->getModuleLink('allinone_rewards', 'sponsorship', [], true)}">
<div>
	<table class="std">
		<thead>
			<tr>
