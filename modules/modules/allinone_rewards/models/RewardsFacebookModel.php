...
class RewardsFacebookModel extends ObjectModel
{
	...
	private function _sendMail($action) {
		...
		$title = ($action == 'Like' ? $module->l2('Someone liked your shop on Facebook', null, strtolower(get_class($this))) : $module->l2('Someone unliked your shop on Facebook', null, strtolower(get_class($this))));
		$data =  array(
			'{action}'		=> $module->l2($action, null, strtolower(get_class($this))),
			'{title}'		=> $title,
			'{customer}'	=> $module->l2('Not identified', null, strtolower(get_class($this)))
		);
		...
		$module->sendMail((int)Configuration::get('PS_LANG_DEFAULT'), 'facebook-admin', $title, $data, strval(Configuration::get('PS_SHOP_EMAIL')), NULL);
	}
}