...
class RewardsModel extends ObjectModel
{
	...
	// renvoie le nombre de crédits gagnés pour une commande, dans la devise de la commande
	public static function getOrderPriceForReward($order, $bDiscountAllowed, $allowedCategories = NULL)
	{
		...
		return self::getCartPriceForReward(new Cart((int)$order->id_cart), $bDiscountAllowed, $allowedCategories, NULL, $returned);
	}
	...
	// renvoie le nombre de crédits gagnés pour un panier, dans la devise du panier
	public static function getCartPriceForReward($cart, $bDiscountAllowed, $allowedCategories = NULL, $newProduct = NULL, $returned = NULL)
	{
		$benefits = false;
		$total = 0;
		$context = Context::getContext();
		...
		foreach ($cartProducts AS $product)
		{
			...
			if ($benefits)
				$total += ($product['price'] - ((float)$product['wholesale_price'] * (float)$cart_currency->conversion_rate)) * (int)($product['cart_quantity'] - (isset($returned[(int)$product['id_product'].'_'.(int)$product['id_product_attribute']]) ? (int)$returned[(int)$product['id_product'].'_'.(int)$product['id_product_attribute']] : 0));
			else
				$total += ($taxesEnabled == PS_TAX_EXC ? $product['price'] : $product['price_wt'])* (int)($product['cart_quantity'] - (isset($returned[(int)$product['id_product'].'_'.(int)$product['id_product_attribute']]) ? (int)$returned[(int)$product['id_product'].'_'.(int)$product['id_product_attribute']] : 0));
		}
	...
	public static function getAllByIdCustomer($id_customer, $admin = false, $onlyValidate = false, $pagination = false, $nb = 10, $page = 1, $currency = NULL)
	{
		...
		$query = '
		SELECT r.id_order AS id_order, r.date_add AS date, (o.total_paid - o.total_shipping) AS total_without_shipping, o.id_currency, r.credits, r.id_reward, r.id_reward_state, r.plugin, r.reason, rsl.name AS state
		FROM `'._DB_PREFIX_.'rewards` r
	...
	public static function createDiscount($credits)
	{
		...
		$configurations = Configuration::getMultiple(array(
			'REWARDS_VOUCHER_PREFIX',
			'REWARDS_VOUCHER_DURATION',
			'REWARDS_VOUCHER_BEHAVIOR',
			'REWARDS_DISPLAY_CART',
			'REWARDS_MINIMAL',
			'REWARDS_MINIMAL_TAX',
			'REWARDS_MINIMAL_SHIPPING',
			'REWARDS_VOUCHER_CATEGORY',
			'REWARDS_VOUCHER_CUMUL_S'
		));

		/* Generate a discount code */
		$code = NULL;
		do $code = $configurations['REWARDS_VOUCHER_PREFIX'].Tools::passwdGen(6);
		...
		$cartRule->date_to = date('Y-m-d H:i:s', time() + (int)$configurations['REWARDS_VOUCHER_DURATION']*24*60*60);
		$cartRule->description = Configuration::get('REWARDS_VOUCHER_DETAILS', (int)$context->language->id);
		$cartRule->quantity = 1;
		$cartRule->quantity_per_user = 1;
		$cartRule->highlight = (int)$configurations['REWARDS_DISPLAY_CART'];
		$cartRule->partial_use = (int)$configurations['REWARDS_VOUCHER_BEHAVIOR'];
		$cartRule->code = $code;
		$cartRule->active = 1;
		...
		$cartRule->minimum_amount = (float)$configurations['REWARDS_MINIMAL'];
		$cartRule->minimum_amount_tax = (int)$configurations['REWARDS_MINIMAL_TAX'];
		$cartRule->minimum_amount_currency = (int)Configuration::get('PS_CURRENCY_DEFAULT');
		$cartRule->minimum_amount_shipping = (int)$configurations['REWARDS_MINIMAL_SHIPPING'];
		$cartRule->cart_rule_restriction = (int)(!(bool)$configurations['REWARDS_VOUCHER_CUMUL_S']);

		$languages = Language::getLanguages(true);
		$default_text = Configuration::get('REWARDS_VOUCHER_DETAILS', (int)(Configuration::get('PS_LANG_DEFAULT')));
		foreach ($languages AS $language)
		{
			$text = Configuration::get('REWARDS_VOUCHER_DETAILS', (int)($language['id_lang']));
			$cartRule->name[(int)($language['id_lang'])] = $text ? strval($text) : strval($default_text);
		}

		$categories = explode(',', $configurations['REWARDS_VOUCHER_CATEGORY']);
		...
		if ((int)$configurations['REWARDS_VOUCHER_CUMUL_S'])
		{
			Db::getInstance()->execute('
	...
	static public function isCustomerAllowed($id_customer, $groups)
	{
		$customer = new Customer($id_customer);
		if (Validate::isLoadedObject($customer)) {
			$allowed_groups = explode(',', $groups);
