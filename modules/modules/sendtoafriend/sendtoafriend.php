...
class sendToAFriend extends Module
{
	...
	{
	 	return (parent::install() && $this->registerHook('extraLeft') && $this->registerHook('customSocialProduct') && $this->registerHook('header'));
	}
	...
	public function hookCustomSocialProduct($params)
	{
		/* Product informations */
		$product = new Product((int)Tools::getValue('id_product'), false, $this->context->language->id);
		$image = Product::getCover((int)$product->id);
		...
		return $this->display(__FILE__, 'sendtoafriend-custom.tpl');
	}
