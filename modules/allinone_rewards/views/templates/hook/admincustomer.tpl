...
<div class="{if $version16}col-lg-12{else}clear{/if}" id="admincustomer">
{if $version16}
	<div class="panel">
...
{/if}
		{l s='Value' mod='allinone_rewards'} <input name="new_reward_value" type="text" size="6" value="{$new_reward_value}" style="text-align: right; display: inline; width: auto"/> {$sign}&nbsp;&nbsp;&nbsp;&nbsp;
	...
	{foreach from=$rewards item=reward name=myLoop}
		{assign var="bUpdate" value="{in_array($reward['id_reward_state'], $states_for_update)}"}
			<tr class="{if ($smarty.foreach.myLoop.iteration % 2) == 0}alt_row{/if}">
				...
				<td>{if $bUpdate}
					<select name="reward_state_{$reward['id_reward']}" style="display: inline; width: auto">
