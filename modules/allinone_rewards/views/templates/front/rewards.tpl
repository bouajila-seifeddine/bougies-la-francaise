...
{/if}
	...
	<div id="general_txt" style="padding-bottom: 20px">{$general_txt}</div>

{if $return_days > 0}
	<p>{l s='Rewards will be available ' mod='allinone_rewards'} {$return_days} {l s='days after the validation of each order.' mod='allinone_rewards'}</p>
{/if}
	<table class="std">
		<thead>
			<tr>
...
<div id="pagination" class="pagination">
	...
	<ul class="pagination">
		...
		<li id="pagination_next"><a href="{$pagination_link}p={$p_next}&n={$nbpagination}">{l s='Next' mod='allinone_rewards'}&nbsp;&raquo;</a></li>
			{else}
...
<div id="payment" style="clear: both">
	...
	<form id="payment_form" class="std" method="post" action="{$pagination_link}" enctype="multipart/form-data" style="display: {if isset($smarty.post.payment_details)}block{else}none{/if}">
		<fieldset>
			<div id="payment_txt">{$payment_txt}</div>
