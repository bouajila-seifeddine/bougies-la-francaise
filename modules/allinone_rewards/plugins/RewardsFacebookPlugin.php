...
class RewardsFacebookPlugin extends RewardsGenericPlugin
{
	...
	public function getContent()
	{
		...
		foreach ($currencies as $currency) {
			$html .= '
								<tr>
									...
									<td align="left"><input '. ((int)$currency['id_currency'] == (int)Configuration::get('PS_CURRENCY_DEFAULT') ? 'class="currency_default"' : '') . ' type="text" size="8" maxlength="8" name="facebook_reward_value['.(int)($currency['id_currency']).']" id="facebook_reward_value['.(int)($currency['id_currency']).']" value="'.Tools::getValue('facebook_reward_value['.(int)($currency['id_currency']).']', Configuration::get('RFACEBOOK_REWARD_VALUE_'.(int)($currency['id_currency']))).'" /> <label class="t">'.$currency['sign'].'</label>'.((int)$currency['id_currency'] != (int)Configuration::get('PS_CURRENCY_DEFAULT') ? ' <a href="#" onClick="return convertCurrencyValue(this, \'facebook_reward_value\', '.$currency['conversion_rate'].')"><img src="'._MODULE_DIR_.'allinone_rewards/images/convert.gif" style="vertical-align: middle !important"></a>' : '').'</td>
								</tr>';
		}
		$html .= '
							</table>
		...
		foreach ($languages as $language) {
			$html .= '
							<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $defaultLanguage ? 'block' : 'none').'; float: left;">
								<input size="30" type="text" name="facebook_voucher_details['.$language['id_lang'].']" value="'.(isset($_POST['facebook_voucher_details'][(int)$language['id_lang']]) ? $_POST['facebook_voucher_details'][(int)$language['id_lang']] : Configuration::get('RFACEBOOK_VOUCHER_DETAILS', (int)$language['id_lang'])).'" />
							</div>';
		}
		$html .= '
						</div>
		...
		$index =  isset($_POST['facebook_voucher_categories']) ? $_POST['facebook_voucher_categories'] : explode(',', Configuration::get('RFACEBOOK_VOUCHER_CATEGORIES'));
		$current = current(current($categories));
		$html .= $this->instance->recurseCategoryForInclude('facebook_voucher_categories', $index, $categories, $current, $current['infos']['id_category']);
		$html .= '
							</table>
						</div>
	...
	{
		$this->context->smarty->assign('version16', version_compare(_PS_VERSION_, '1.6', '>='));
	...
	{
		$hooks = explode(',', Configuration::get('RFACEBOOK_HOOKS'));
		if (is_array($hooks) && in_array('shoppingcart', $hooks)) {
