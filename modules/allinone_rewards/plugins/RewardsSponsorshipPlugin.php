...
class RewardsSponsorshipPlugin extends RewardsGenericPlugin
{
	...
	public function install()
	{
		...
		|| !$this->registerHook('displayAdminCustomers') || !$this->registerHook('displayAdminOrder')
	...
	{
		$this->configuration['reward_type'] = explode(',', Configuration::get('RSPONSORSHIP_REWARD_TYPE_S'));
		$this->configuration['reward_percentage'] = explode(',', Configuration::get('RSPONSORSHIP_REWARD_PERCENTAGE'));
		$this->configuration['unlimited'] = ((int) Configuration::get('RSPONSORSHIP_UNLIMITED_LEVELS') > 0);

		$currencies = Currency::getCurrencies();
		foreach ($currencies as $currency) {
			$values = explode(',', Configuration::get('RSPONSORSHIP_REWARD_VALUE_S_'.$currency['id_currency']));
			foreach($this->configuration['reward_percentage'] as $level => $percentage) {
				$this->configuration['reward_value'][$level][$currency['id_currency']] = $values[$level];
			}
		}
	}
	...
	{
		return Configuration::get('RSPONSORSHIP_ACTIVE');
	}
	...
	public function postProcess()
	{
		if (Tools::isSubmit('submitSponsorship')) {
			Configuration::updateValue('RSPONSORSHIP_ACTIVE', (int)Tools::getValue('sponsorship_active'));
			Configuration::updateValue('RSPONSORSHIP_REDIRECT', Tools::getValue('sponsorship_redirect'));
			Configuration::updateValue('RSPONSORSHIP_ORDER_QUANTITY_S', (int)Tools::getValue('order_quantity_s'));
			Configuration::updateValue('RSPONSORSHIP_VOUCHER_DETAILS', Tools::getValue('description_gc'));
			Configuration::updateValue('RSPONSORSHIP_VOUCHER_PREFIX_GC', Tools::getValue('voucher_prefix_gc'));
			Configuration::updateValue('RSPONSORSHIP_VOUCHER_DURATION_GC', (int)Tools::getValue('voucher_duration_gc'));
			Configuration::updateValue('RSPONSORSHIP_CATEGORIES_GC', implode(",", Tools::getValue('voucher_categories_gc')));
			Configuration::updateValue('RSPONSORSHIP_NB_FRIENDS', (int)Tools::getValue('nb_friends'));
			Configuration::updateValue('RSPONSORSHIP_CUMUL_GC', (int)Tools::getValue('cumulative_voucher_gc'));
			Configuration::updateValue('RSPONSORSHIP_VOUCHER_BEHAVIOR', (int)Tools::getValue('voucher_behavior_gc'));
			Configuration::updateValue('RSPONSORSHIP_QUANTITY_GC', (int)Tools::getValue('voucher_quantity_gc'));
			Configuration::updateValue('RSPONSORSHIP_MINIMAL_TAX_GC', (int)Tools::getValue('include_tax_gc'));
			Configuration::updateValue('RSPONSORSHIP_REWARD_S', (int)Tools::getValue('reward_s'));
			Configuration::updateValue('RSPONSORSHIP_ON_EVERY_ORDER', (int)Tools::getValue('reward_on_every_order'));
			Configuration::updateValue('RSPONSORSHIP_DISCOUNT_GC', (int)Tools::getValue('discount_gc'));
			Configuration::updateValue('RSPONSORSHIP_UNLOCK_SHIPPING', (int)Tools::getValue('unlock_shipping'));
			Configuration::updateValue('RSPONSORSHIP_DISCOUNT_TYPE_GC', (int)Tools::getValue('discount_type_gc'));
			Configuration::updateValue('RSPONSORSHIP_FREESHIPPING_GC', (int)Tools::getValue('freeshipping_gc'));
			Configuration::updateValue('RSPONSORSHIP_DISCOUNTED_ALLOWED', (int)Tools::getValue('rsponsorship_discounted_allowed'));
			...
			Configuration::updateValue('RSPONSORSHIP_SHARE_IMAGE_URL', Tools::getValue('share_image_url'));
			...
			foreach (Tools::getValue('discount_value_gc') as $id_currency => $discount_value_gc) {
				if ((int)Tools::getValue('discount_type_gc') == 0)
					Configuration::updateValue('RSPONSORSHIP_VOUCHER_VALUE_GC_'.(int)($id_currency), 0);
				else
					Configuration::updateValue('RSPONSORSHIP_VOUCHER_VALUE_GC_'.(int)($id_currency), (float)($discount_value_gc));
			}
			foreach (Tools::getValue('minimum_value_gc') as $id_currency => $minimum_value_gc)
				Configuration::updateValue('RSPONSORSHIP_MINIMUM_VALUE_GC_'.(int)($id_currency), (float)($minimum_value_gc));
			foreach (Tools::getValue('unlock_gc') as $id_currency => $unlock_gc)
				Configuration::updateValue('RSPONSORSHIP_UNLOCK_GC_'.(int)($id_currency), (float)($unlock_gc));
			...
			Configuration::updateValue('RSPONSORSHIP_REWARD_TYPE_S', implode(",", Tools::getValue('reward_type_s')));
			Configuration::updateValue('RSPONSORSHIP_REWARD_PERCENTAGE', implode(",", Tools::getValue('reward_percentage')));
			foreach (Tools::getValue('reward_value_s') as $id_currency => $reward_value_s) {
				if (is_array($reward_value_s)) {
					Configuration::updateValue('RSPONSORSHIP_REWARD_VALUE_S_'.(int)($id_currency), implode(",", $reward_value_s));
				}
			}
			...
			Configuration::updateValue('RSPONSORSHIP_ACCOUNT_TXT', Tools::getValue('account_txt'), true);
			Configuration::updateValue('RSPONSORSHIP_ORDER_TXT', Tools::getValue('order_txt'), true);
			Configuration::updateValue('RSPONSORSHIP_POPUP_TXT', Tools::getValue('popup_txt'), true);
			Configuration::updateValue('RSPONSORSHIP_RULES_TXT', Tools::getValue('rules_txt'), true);
			if (version_compare(_PS_VERSION_, '1.5.2', '<')) {
				Configuration::set('RSPONSORSHIP_ACCOUNT_TXT', Tools::getValue('account_txt'));
				Configuration::set('RSPONSORSHIP_ORDER_TXT', Tools::getValue('order_txt'));
				Configuration::set('RSPONSORSHIP_POPUP_TXT', Tools::getValue('popup_txt'));
				Configuration::set('RSPONSORSHIP_RULES_TXT', Tools::getValue('rules_txt'));
			}
		}
	...
	private function _displayForm()
	{
		...
		<div class="tabs" style="display: none">
			<ul>
				...
				<li><a href="'.$_SERVER['REQUEST_URI'].'&stats=1">'.$this->l('Statistics').'</a></li>
			</ul>
			<div id="tabs-'.$this->name.'-1">
				...
				<fieldset>
					...
					<div class="margin-form">
						...
						<input type="radio" id="sponsorship_active_on" name="sponsorship_active" value="1" '.(Tools::getValue('sponsorship_active', Configuration::get('RSPONSORSHIP_ACTIVE')) == 1 ? 'checked="checked"' : '').' /> <label class="t" for="sponsorship_active_on">' . $this->l('Yes') . '</label>
						...
						<input type="radio" id="sponsorship_active_off" name="sponsorship_active" value="0" '.(Tools::getValue('sponsorship_active', Configuration::get('RSPONSORSHIP_ACTIVE')) == 0 ? 'checked="checked"' : '').' /> <label class="t" for="sponsorship_active_off">' . $this->l('No') . '</label>
					</div>
					<div class="clear"></div>
					...
					<div class="margin-form">
						...
						<input type="radio" id="reward_s_on" name="reward_s" value="1" '.(Tools::getValue('reward_s', Configuration::get('RSPONSORSHIP_REWARD_S')) == 1 ? 'checked="checked"' : '').' /> <label class="t" for="reward_s_on">' . $this->l('Yes') . '</label>
						...
						<input type="radio" id="reward_s_off" name="reward_s" value="0" '.(Tools::getValue('reward_s', Configuration::get('RSPONSORSHIP_REWARD_S')) == 0 ? 'checked="checked"' : '').' /> <label class="t" for="reward_s_off">' . $this->l('No') . '</label>
					</div>
					<div class="clear"></div>
					...
					<div class="margin-form">
						...
						<input type="radio" id="discount_gc_on" name="discount_gc" value="1" '.(Tools::getValue('discount_gc', Configuration::get('RSPONSORSHIP_DISCOUNT_GC')) == 1 ? 'checked="checked"' : '').' /> <label class="t" for="discount_gc_on">' . $this->l('Yes') . '</label>
						...
						<input type="radio" id="discount_gc_off" name="discount_gc" value="0" '.(Tools::getValue('discount_gc', Configuration::get('RSPONSORSHIP_DISCOUNT_GC')) == 0 ? 'checked="checked"' : '').' /> <label class="t" for="discount_gc_off">' . $this->l('No') . '</label>
					</div>
					<div class="clear"></div>
					...
					<div class="margin-form">
						<input type="text" size="60" name="share_image_url" id="share_image_url" value="'.(Tools::getValue('share_url_image', Configuration::get('RSPONSORSHIP_SHARE_IMAGE_URL'))).'" />
					</div>
					<div class="clear"></div>
					<label>'.$this->l('Number of lines displayed in the invitation form').'</label>
					<div class="margin-form">
						<input type="text" size="3" maxlength="3" name="nb_friends" id="nb_friends" value="'.(int)(Tools::getValue('nb_friends', Configuration::get('RSPONSORSHIP_NB_FRIENDS'))).'" />
					</div>
					<div class="clear"></div>
					<label>'.$this->l('Number of orders to be able to become a sponsor (0 is allowed)').'</label>
					<div class="margin-form">
						<input type="text" size="3" maxlength="3" name="order_quantity_s" id="order_quantity_s" value="'.(int)(Tools::getValue('order_quantity_s', Configuration::get('RSPONSORSHIP_ORDER_QUANTITY_S'))).'" />
					</div>
					<div class="clear"></div>
		...
		foreach($groups as $group) {
			$this->html .= '<option '.(is_array($allowed_groups) && in_array($group['id_group'], $allowed_groups) ? 'selected':'').' value="'.$group['id_group'].'"> '.$group['name'].'</option>';
		}
		$this->html .= '
						</select>
					</div>
				...
				<fieldset class="sponsor_optional">
					...
					<div class="margin-form">
						<label class="t" for="discount_gc_on"><img src="../img/admin/enabled.gif" alt="'.$this->l('Enabled').'" title="'.$this->l('Yes').'" /></label>
						<input type="radio" id="reward_on_every_order_on" name="reward_on_every_order" value="1" '.(Tools::getValue('reward_on_every_order', Configuration::get('RSPONSORSHIP_ON_EVERY_ORDER')) == 1 ? 'checked="checked"' : '').' /> <label class="t" for="discount_gc_on">'.$this->l('Yes').'</label>
						<label class="t" for="discount_gc_off" style="margin-left: 10px"><img src="../img/admin/disabled.gif" alt="'.$this->l('Disabled').'" title="'.$this->l('No').'" /></label>
						<input type="radio" id="reward_on_every_order_off" name="reward_on_every_order" value="0" '.(Tools::getValue('reward_on_every_order', Configuration::get('RSPONSORSHIP_ON_EVERY_ORDER')) == 0 ? 'checked="checked"' : '').' /> <label class="t" for="discount_gc_off">'.$this->l('No, only for the first one').'</label>
					</div>
					<div class="clear"></div>
					...
					<div class="margin-form">
						...
						<input type="radio" id="rsponsorship_discounted_allowed_on" name="rsponsorship_discounted_allowed" value="1" '.(Tools::getValue('rsponsorship_discounted_allowed', Configuration::get('RSPONSORSHIP_DISCOUNTED_ALLOWED')) == 1 ? 'checked="checked"' : '').' /> <label class="t" for="rsponsorship_discounted_allowed_on">' . $this->l('Yes') . '</label>
						...
						<input type="radio" id="rsponsorship_discounted_allowed_off" name="rsponsorship_discounted_allowed" value="0" '.(Tools::getValue('rsponsorship_discounted_allowed', Configuration::get('RSPONSORSHIP_DISCOUNTED_ALLOWED')) == 0 ? 'checked="checked"' : '').' /> <label class="t" for="rsponsorship_discounted_allowed_off">' . $this->l('No') . '</label>
					</div>
					...
					<div class="margin-form">
						<input type="radio" id="unlock_shipping_on" name="unlock_shipping" value="1" '.(Tools::getValue('unlock_shipping', Configuration::get('RSPONSORSHIP_UNLOCK_SHIPPING')) == 1 ? 'checked="checked"' : '').' /> <label class="t" for="unlock_shipping_on" style="padding-right: 10px">' . $this->l('Total with shipping included') . '</label>
						<input type="radio" id="unlock_shipping_off" name="unlock_shipping" value="0" '.(Tools::getValue('unlock_shipping', Configuration::get('RSPONSORSHIP_UNLOCK_SHIPPING')) == 0 ? 'checked="checked"' : '').' /> <label class="t" for="unlock_shipping_off">' . $this->l('Total without shipping') . '</label>
					</div>
					<div class="clear">';
		foreach ($currencies as $currency) {
			$this->html .= '
						...
						<div class="margin-form"><input '. ((int)$currency['id_currency'] == (int)Configuration::get('PS_CURRENCY_DEFAULT') ? 'class="currency_default"' : '') . ' type="text" size="8" maxlength="8" name="unlock_gc['.(int)($currency['id_currency']).']" id="unlock_gc['.(int)($currency['id_currency']).']" value="'.Tools::getValue('unlock_gc['.(int)($currency['id_currency']).']', Configuration::get('RSPONSORSHIP_UNLOCK_GC_'.(int)($currency['id_currency']))).'" /> <label class="t">'.$currency['sign'].'</label>'.((int)$currency['id_currency'] != (int)Configuration::get('PS_CURRENCY_DEFAULT') ? ' <a href="#" onClick="return convertCurrencyValue(this, \'unlock_gc\', '.$currency['conversion_rate'].')"><img src="'._MODULE_DIR_.'allinone_rewards/images/convert.gif" style="vertical-align: middle !important"></a>' : '').'</div>';
		}
		$this->html .= '
					</div>
					<div class="clear" style="padding-top: 10px"></div>
		...
		foreach($this->configuration['reward_type'] as $level => $reward_type) {
			$this->html .= '
			...
			foreach ($currencies as $currency) {
				$this->html .= '
							...
							<div class="margin-form"><input '. ((int)$currency['id_currency'] == (int)Configuration::get('PS_CURRENCY_DEFAULT') ? 'class="currency_default"' : '') . ' type="text" size="8" maxlength="8" name="reward_value_s['.(int)($currency['id_currency']).']['.$level.']" value="'.$this->configuration['reward_value'][$level][$currency['id_currency']].'" /> <label class="t">'.$currency['sign'].'</label>'.((int)$currency['id_currency'] != (int)Configuration::get('PS_CURRENCY_DEFAULT') ? ' <a href="#" onClick="return convertCurrencyValue(this, \'reward_value_s\', '.$currency['conversion_rate'].')"><img src="'._MODULE_DIR_.'allinone_rewards/images/convert.gif" style="vertical-align: middle !important"></a>' : '').'</div>';
			}
			$this->html .= '
						</div>
						<div class="clear"></div>
						<div class="reward_percentage">
							...
							<div class="margin-form">
								<input type="text" size="3" name="reward_percentage['.$level.']" value="'.$this->configuration['reward_percentage'][$level].'" /> %
							</div>
						</div>
					</div>';
		}
		$this->html .= '
					<div class="clear"></div>
					<label>'.$this->l('All next levels will use the settings from level').' <span id="unlimited_level">'.count($this->configuration['reward_type']).'</span></label>
					<div class="margin-form">
						<label class="t" for="unlimited_levels_on"><img src="../img/admin/enabled.gif" alt="'.$this->l('Enabled').'" title="'.$this->l('Yes').'" /></label>
						<input type="radio" id="unlimited_levels_on" name="unlimited_levels" value="1" '.(Tools::getValue('unlimited_levels', $this->configuration['unlimited']) ? 'checked="checked"' : '').' /> <label class="t" for="unlimited_levels_on">' . $this->l('Yes') . '</label>
		...
		foreach ($languages as $language) {
			$this->html .= '
						<div class="lang_'.$language['id_lang'].'" id="descgc_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $defaultLanguage ? 'block' : 'none').'; float: left;">
							<input size="30" type="text" name="description_gc['.$language['id_lang'].']" value="'.(isset($_POST['description_gc'][(int)($language['id_lang'])]) ? htmlentities($_POST['description_gc'][(int)($language['id_lang'])], ENT_QUOTES, 'utf-8') : htmlentities(Configuration::get('RSPONSORSHIP_VOUCHER_DETAILS', (int)$language['id_lang']), ENT_QUOTES, 'utf-8')).'" />
						</div>';
		}
		$this->html .= '
					</div>
					...
					<div class="margin-form">
						<input type="text" size="10" maxlength="10" id="voucher_prefix_gc" name="voucher_prefix_gc" value="'.Tools::getValue('voucher_prefix_gc', Configuration::get('RSPONSORSHIP_VOUCHER_PREFIX_GC')).'" />
					</div>
					<div class="clear"></div>
					<label>'.$this->l('Validity of the voucher (in days)').'</label>
					<div class="margin-form">
						<input type="text" size="4" maxlength="4" id="voucher_duration_gc" name="voucher_duration_gc" value="'.Tools::getValue('voucher_duration_gc', Configuration::get('RSPONSORSHIP_VOUCHER_DURATION_GC')).'" />
					</div>
					<div class="clear"></div>
					<label>'.$this->l('Number of times the voucher can be used by the sponsored friend').'</label>
					<div class="margin-form">
						<input type="text" size="4" maxlength="4" id="voucher_quantity_gc" name="voucher_quantity_gc" value="'.Tools::getValue('voucher_quantity_gc', Configuration::get('RSPONSORSHIP_QUANTITY_GC')).'" />
					</div>
					<div class="clear"></div>
					...
					<div class="margin-form">
						...
						<input type="radio" id="freeshipping_gc_on" name="freeshipping_gc" value="1" '.(Tools::getValue('freeshipping_gc', Configuration::get('RSPONSORSHIP_FREESHIPPING_GC')) == 1 ? 'checked="checked"' : '').' /> <label class="t" for="freeshipping_gc_on">' . $this->l('Yes') . '</label>
						...
						<input type="radio" id="freeshipping_gc_off" name="freeshipping_gc" value="0" '.(Tools::getValue('freeshipping_gc', Configuration::get('RSPONSORSHIP_FREESHIPPING_GC')) == 0 ? 'checked="checked"' : '').' /> <label class="t" for="freeshipping_gc_off">' . $this->l('No') . '</label>
					</div>
					<div class="clear"></div>
					<label>'.$this->l('Apply a discount').'</label>
					<div class="margin-form">
						<input onClick="$(\'#sponsored td.voucher_value\').html(\'' . $this->l('Voucher %') . '\');$(\'#sponsored td.value_cols\').show();$(\'#behavior_gc\').hide();$(\'#voucher_behavior_gc\').val(0)" type="radio" id="discount_type_gc_1" name="discount_type_gc" value="1" '.(Tools::getValue('discount_type_gc', Configuration::get('RSPONSORSHIP_DISCOUNT_TYPE_GC')) == 1 ? 'checked="checked"' : '').' /> <label class="t" for="discount_type_gc_1" style="padding-right: 10px">'.$this->l('Percentage').'</label>
						<input onClick="$(\'#sponsored td.value_cols\').hide();$(\'#sponsored td.voucher_value\').html(\'' . $this->l('Voucher value') . '\');$(\'#sponsored td.value_cols\').show();$(\'#behavior_gc\').show()" type="radio" id="discount_type_gc_2" name="discount_type_gc" value="2" '.(Tools::getValue('discount_type_gc', Configuration::get('RSPONSORSHIP_DISCOUNT_TYPE_GC')) == 2 ? 'checked="checked"' : '').' /> <label class="t" for="discount_type_gc_2" style="padding-right: 10px">'.$this->l('Amount').'</label>
						<input onClick="$(\'#sponsored td.value_cols\').hide();$(\'#behavior_gc\').hide();$(\'#voucher_behavior_gc\').val(0)" type="radio" id="discount_type_gc_0" name="discount_type_gc" value="0" '.(Tools::getValue('discount_type_gc', Configuration::get('RSPONSORSHIP_DISCOUNT_TYPE_GC')) == 0 ? 'checked="checked"' : '').' /> <label class="t" for="discount_type_gc_0">'.$this->l('None').'</label>
					</div>
					<div class="clear"></div>
		...
		$index =  isset($_POST['voucher_categories_gc']) ? $_POST['voucher_categories_gc'] : explode(',', Configuration::get('RSPONSORSHIP_CATEGORIES_GC'));
		$current = current(current($categories));
		$this->html .= $this->instance->recurseCategoryForInclude('voucher_categories_gc', $index, $categories, $current, $current['infos']['id_category']);
		$this->html .= '
						</table>
					</div>
					<div id="behavior_gc" style="display:'.(Tools::getValue('discount_type_gc', Configuration::get('RSPONSORSHIP_DISCOUNT_TYPE_GC')) == 2 ? 'block' : 'none').'">
						<div class="clear"></div>
						<label>'.$this->l('If the voucher is not depleted when used').'</label>&nbsp;
						<div class="margin-form">
							<select name="voucher_behavior_gc" id="voucher_behavior_gc">
								<option '.(!Tools::getValue('voucher_behavior_gc', (int)Configuration::get('RSPONSORSHIP_VOUCHER_BEHAVIOR')) ?'selected':'').' value="0">'.$this->l('Cancel the remaining amount').'</option>
								<option '.(Tools::getValue('voucher_behavior_gc', (int)Configuration::get('RSPONSORSHIP_VOUCHER_BEHAVIOR')) ?'selected':'').' value="1">'.$this->l('Create a new voucher with remaining amount').'</option>
							</select>
						</div>
					</div>
					<div class="clear"></div>
					...
					<div class="margin-form">
						...
						<input type="radio" id="cumulative_voucher_gc_on" name="cumulative_voucher_gc" value="1" '.(Tools::getValue('cumulative_voucher_gc', Configuration::get('RSPONSORSHIP_CUMUL_GC')) == 1 ? 'checked="checked"' : '').' /> <label class="t" for="cumulative_voucher_gc_on">' . $this->l('Yes') . '</label>
						...
						<input type="radio" id="cumulative_voucher_gc_off" name="cumulative_voucher_gc" value="0" '.(Tools::getValue('cumulative_voucher_gc', Configuration::get('RSPONSORSHIP_CUMUL_GC')) == 0 ? 'checked="checked"' : '').' /> <label class="t" for="cumulative_voucher_gc_off">' . $this->l('No') . '</label>
					</div>
					<div class="clear"></div>
					...
					<div class="margin-form">
						...
						<input type="radio" id="include_tax_gc_on" name="include_tax_gc" value="1" '.(Tools::getValue('include_tax_gc', Configuration::get('RSPONSORSHIP_MINIMAL_TAX_GC')) == 1 ? 'checked="checked"' : '').' /> <label class="t" for="include_tax_gc_on">' . $this->l('Yes') . '</label>
						...
						<input type="radio" id="include_tax_gc_off" name="include_tax_gc" value="0" '.(Tools::getValue('include_tax_gc', Configuration::get('RSPONSORSHIP_MINIMAL_TAX_GC')) == 0 ? 'checked="checked"' : '').' /> <label class="t" for="include_tax_gc_off">' . $this->l('No') . '</label>
					</div>
					<div class="clear"></div>
					<div>
						<table>
							<tr>
								...
								<td width="165" class="voucher_value value_cols" style="font-weight: bold; display:'.(Tools::getValue('discount_type_gc', Configuration::get('RSPONSORSHIP_DISCOUNT_TYPE_GC')) == 0 ? 'none' : 'block').'">' . (Tools::getValue('discount_type_gc', Configuration::get('RSPONSORSHIP_DISCOUNT_TYPE_GC')) == 1 ? $this->l('Voucher %') : $this->l('Voucher value')) . '</td>
		...
		foreach ($currencies as $currency) {
			$this->html .= '
							<tr>
								...
								<td align="left" class="value_cols" style="display:'.(Tools::getValue('discount_type_gc', Configuration::get('RSPONSORSHIP_DISCOUNT_TYPE_GC')) == 0 ? 'none' : 'block').'"><input '. ((int)$currency['id_currency'] == (int)Configuration::get('PS_CURRENCY_DEFAULT') ? 'class="currency_default"' : '') . ' type="text" size="8" maxlength="8" name="discount_value_gc['.(int)($currency['id_currency']).']" id="discount_value_gc['.(int)($currency['id_currency']).']" value="'.Tools::getValue('discount_value_gc['.(int)($currency['id_currency']).']', Configuration::get('RSPONSORSHIP_VOUCHER_VALUE_GC_'.(int)($currency['id_currency']))).'" />'.((int)$currency['id_currency'] != (int)Configuration::get('PS_CURRENCY_DEFAULT') ? ' <a href="#" onClick="return convertCurrencyValue(this, \'discount_value_gc\', '.$currency['conversion_rate'].')"><img src="'._MODULE_DIR_.'allinone_rewards/images/convert.gif" style="vertical-align: middle !important"></a>' : '').'</td>
								<td class="value_cols">&nbsp;</td>
								<td align="left"><input '. ((int)$currency['id_currency'] == (int)Configuration::get('PS_CURRENCY_DEFAULT') ? 'class="currency_default"' : '') . ' type="text" size="8" maxlength="8" name="minimum_value_gc['.(int)($currency['id_currency']).']" id="minimum_value_gc['.(int)($currency['id_currency']).']" value="'.Tools::getValue('minimum_value_gc['.(int)($currency['id_currency']).']', Configuration::get('RSPONSORSHIP_MINIMUM_VALUE_GC_'.(int)($currency['id_currency']))).'" />'.((int)$currency['id_currency'] != (int)Configuration::get('PS_CURRENCY_DEFAULT') ? ' <a href="#" onClick="return convertCurrencyValue(this, \'minimum_value_gc\', '.$currency['conversion_rate'].')"><img src="'._MODULE_DIR_.'allinone_rewards/images/convert.gif" style="vertical-align: middle !important"></a>' : '').'</td>
							</tr>';
		}
		$this->html .= '
						</table>
		...
		foreach ($languages AS $language) {
			$this->html .= '
							<div class="lang_'.$language['id_lang'].'" id="account_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $defaultLanguage ? 'block' : 'none').';float: left;">
								<textarea class="rte autoload_rte" cols="120" rows="25" name="account_txt['.$language['id_lang'].']">'.Configuration::get('RSPONSORSHIP_ACCOUNT_TXT', (int)$language['id_lang']).'</textarea>
							</div>';
		}
		$this->html .= '
		...
		foreach ($languages AS $language) {
			$this->html .= '
							<div class="lang_'.$language['id_lang'].'" id="order_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $defaultLanguage ? 'block' : 'none').';float: left;">
								<textarea class="rte autoload_rte" cols="120" rows="25" name="order_txt['.$language['id_lang'].']">'.Configuration::get('RSPONSORSHIP_ORDER_TXT', (int)$language['id_lang']).'</textarea>
							</div>';
		}
		$this->html .= '
						</div>
		...
		foreach ($languages AS $language) {
			$this->html .= '
							<div class="lang_'.$language['id_lang'].'" id="popup_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $defaultLanguage ? 'block' : 'none').';float: left;">
								<textarea class="rte autoload_rte" cols="120" rows="25" name="popup_txt['.$language['id_lang'].']">'.Configuration::get('RSPONSORSHIP_POPUP_TXT', (int)$language['id_lang']).'</textarea>
							</div>';
		}
		$this->html .= '
						</div>
		...
		foreach ($languages AS $language) {
			$this->html .= '
							<div class="lang_'.$language['id_lang'].'" id="rules_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $defaultLanguage ? 'block' : 'none').';float: left;">
								<textarea class="rte autoload_rte" cols="120" rows="25" name="rules_txt['.$language['id_lang'].']">'.Configuration::get('RSPONSORSHIP_RULES_TXT', (int)$language['id_lang']).'</textarea>
							</div>';
		}
		$this->html .= '
						</div>
	...
	public function hookDisplayHeader()
	{
		...
		if (Configuration::get('RSPONSORSHIP_SHARE_IMAGE_URL') && ($this->context->controller instanceof IndexController || $this->context->controller instanceof AuthController)) {
			$this->context->smarty->assign(array('ogimage' => Configuration::get('RSPONSORSHIP_SHARE_IMAGE_URL')));
	...
	private function _createSponsorship($sponsor, $customer, $force=false, $voucher=true, $currency=0) {
		if (Validate::isLoadedObject($sponsor) && RewardsSponsorshipModel::isCustomerAllowed($sponsor) && $sponsor->email != $customer->email)
		{
			...
			if ($nbOrdersSponsor >= (int)(Configuration::get('RSPONSORSHIP_ORDER_QUANTITY_S')) || $force) {
				...
				if (Configuration::get('RSPONSORSHIP_MAIL_REGISTRATION') || Configuration::get('RSPONSORSHIP_MAIL_REGISTRATION_S')) {
					...
					if (Configuration::get('RSPONSORSHIP_MAIL_REGISTRATION'))
						$this->instance->sendMail((int)Configuration::get('PS_LANG_DEFAULT'), 'sponsorship-registration-admin', $this->l('Sponsorship'), $data, strval(Configuration::get('PS_SHOP_EMAIL')), NULL);
				...
				if (Configuration::get('RSPONSORSHIP_DISCOUNT_GC') && $voucher) {
	...
	{
		$this->_initConf();
		$limit = count($this->configuration['reward_type']) - 1;

		$price = (float)RewardsModel::getOrderPriceForReward($order, Configuration::get('RSPONSORSHIP_DISCOUNTED_ALLOWED'));
		if ($price > 0) {
			...
			foreach($sponsorships as $sponsorship) {
				...
				$indice = $level;
				if (!isset($this->configuration['reward_type'][$level]))
					$indice = $limit;
				$extraParams['type'] = (int)$this->configuration['reward_type'][$indice];
				$extraParams['value'] = (float)($extraParams['type'] == 1 ? $this->configuration['reward_value'][$indice][$order->id_currency] : $this->configuration['reward_percentage'][$indice]);
			...
			if ($bMail && Configuration::get('RSPONSORSHIP_MAIL_ORDER')) {
				...
				$this->instance->sendMail((int)Configuration::get('PS_LANG_DEFAULT'), 'sponsorship-order-admin', $this->l('Sponsorship'), $data, strval(Configuration::get('PS_SHOP_EMAIL')), NULL);
			}
		}
	}
	...
	public function hookActionValidateOrder($params)
	{
		...
		if (Configuration::get('RSPONSORSHIP_REWARD_S')) {
			...
			if ((int)Configuration::get('RSPONSORSHIP_ON_EVERY_ORDER') == 0 && RewardsSponsorshipModel::isAlreadyRewarded($sponsorship->id))
				return false;

			// Shipping included in minimum to unlock sponsor's reward ?
			$total_unlock = (float)$order->total_paid;
			if ((int)Configuration::get('RSPONSORSHIP_UNLOCK_SHIPPING') == 0)
				$total_unlock = (float)$order->total_paid - (float)$order->total_shipping;

			// Check if minimum is reached
			if ($total_unlock >= (float)Configuration::get('RSPONSORSHIP_UNLOCK_GC_' . $order->id_currency)) {
		...
		foreach($sponsorships as $sponsorship) {
			...
			if ($reward->id_reward_state != RewardsStateModel::getConvertId()) {
				...
				if ((float)$oldCredits != (float)$reward->credits) {
					if (!Configuration::get('RSPONSORSHIP_DISCOUNTED_ALLOWED') && (float)$reward->credits == 0)
	...
	{
		if (!$this->popup && $this->context->customer->isLogged() && Validate::isLoadedObject($this->context->customer) && RewardsSponsorshipModel::isCustomerAllowed($this->context->customer)) {
			$stats = $this->context->customer->getStats();
			$nbOrdersCustomer = (int)$stats['nb_orders'];
			// if nb orders required to sponsor is reached
			if ($nbOrdersCustomer >= (int)Configuration::get('RSPONSORSHIP_ORDER_QUANTITY_S')) {
				$this->popup = true;
				$key = 'rewards_sponsor' . Configuration::get('RSPONSORSHIP_POPUP_KEY');
	...
	public function hookDisplayAdminCustomers($params)
	{
		...
		$msg = '';
		if (Tools::isSubmit('submitSponsor') && (int)Tools::getValue('new_sponsor') != 0) {
			...
			if ($this->_createSponsorship($new_sponsor, $customer, true, (bool)Tools::getValue('generate_voucher'), (int)Tools::getValue('generate_currency')))
				$msg = $this->instance->displayConfirmation($this->l('The sponsor has been updated.'));
			else
				$msg = $this->instance->displayError($this->l('The sponsor update failed.'));
		}
