...
class RewardsSponsorshipModel extends ObjectModel
{
	...
	public function registerDiscount($id_currency)
	{
		...
		$configurations = Configuration::getMultiple(array(
			'RSPONSORSHIP_DISCOUNT_TYPE_GC',
			'RSPONSORSHIP_FREESHIPPING_GC',
			'RSPONSORSHIP_CUMUL_GC',
			'RSPONSORSHIP_VOUCHER_BEHAVIOR',
			'RSPONSORSHIP_QUANTITY_GC',
			'RSPONSORSHIP_MINIMAL_TAX_GC',
			'RSPONSORSHIP_VOUCHER_PREFIX_GC',
			'RSPONSORSHIP_VOUCHER_DURATION_GC',
			'RSPONSORSHIP_VOUCHER_VALUE_GC_'.$id_currency,
			'RSPONSORSHIP_MINIMUM_VALUE_GC_'.$id_currency,
			'RSPONSORSHIP_CATEGORIES_GC'
		));

		/* Generate a discount code */
		$code = NULL;
		do $code = $configurations['RSPONSORSHIP_VOUCHER_PREFIX_GC'].Tools::passwdGen(6);
		...
		$cartRule->date_to = date('Y-m-d H:i:s', time() + (int)$configurations['RSPONSORSHIP_VOUCHER_DURATION_GC']*24*60*60);
		$cartRule->description = Configuration::get('RSPONSORSHIP_VOUCHER_DETAILS', (int)$context->language->id);
		$cartRule->quantity = (int)$configurations['RSPONSORSHIP_QUANTITY_GC'];
		$cartRule->quantity_per_user = (int)$configurations['RSPONSORSHIP_QUANTITY_GC'];
		$cartRule->highlight = 1;
		if ((int)$configurations['RSPONSORSHIP_DISCOUNT_TYPE_GC'] == 2)
			$cartRule->partial_use = (int)$configurations['RSPONSORSHIP_VOUCHER_BEHAVIOR'];
		else
			$cartRule->partial_use = 0;
		$cartRule->minimum_amount = (float)$configurations['RSPONSORSHIP_MINIMUM_VALUE_GC_'.$id_currency];
		$cartRule->minimum_amount_tax = (int)$configurations['RSPONSORSHIP_MINIMAL_TAX_GC'];
		$cartRule->minimum_amount_currency = $id_currency;
		$cartRule->minimum_amount_shipping = 0;
		$cartRule->cart_rule_restriction = (int)(!(bool)$configurations['RSPONSORSHIP_CUMUL_GC']);

		if ((int)$configurations['RSPONSORSHIP_DISCOUNT_TYPE_GC'] == 1) {
			$cartRule->reduction_percent = (float)$configurations['RSPONSORSHIP_VOUCHER_VALUE_GC_'.$id_currency];
		} elseif ((int)$configurations['RSPONSORSHIP_DISCOUNT_TYPE_GC'] == 2) {
			$cartRule->reduction_amount = (float)$configurations['RSPONSORSHIP_VOUCHER_VALUE_GC_'.$id_currency];
			$cartRule->reduction_currency = $id_currency;
			$cartRule->reduction_tax = 1;
		}
		if ((int)$configurations['RSPONSORSHIP_FREESHIPPING_GC'] == 1) {
			$cartRule->free_shipping = 1;
		}

		$languages = Language::getLanguages(true);
		$default_text = Configuration::get('RSPONSORSHIP_VOUCHER_DETAILS', (int)(Configuration::get('PS_LANG_DEFAULT')));
		foreach ($languages AS $language)
		{
			$text = Configuration::get('RSPONSORSHIP_VOUCHER_DETAILS', (int)($language['id_lang']));
			$cartRule->name[(int)($language['id_lang'])] = $text ? strval($text) : strval($default_text);
		}

		$categories = explode(',', $configurations['RSPONSORSHIP_CATEGORIES_GC']);
		if (is_array($categories) && count($categories) > 0 && (int)$configurations['RSPONSORSHIP_DISCOUNT_TYPE_GC'] != 0)
		...
		if ($cartRule->add()) {
			...
			if ((int)$configurations['RSPONSORSHIP_CUMUL_GC'])
			{
				Db::getInstance()->execute('
	...
	static public function isSponsorised($idCustomer, $getId=false)
	{
		...
		if (isset($result['id_sponsorship']) && $getId===true)
	...
	static public function getSponsorshipLink()
	{
		$context = Context::getContext();
	...
	static public function decodeSponsorshipLink($value) {
		$id_customer = substr($value, 2, -2);
		$date_add = substr($value, -2) . '-' . substr($value, 0, 2);
		$query = '
			SELECT id_customer
	...
	static public function decodeSponsorshipMailLink($value) {
		$id_sponsorship = substr($value, 3, -2);
		$date_add = substr($value, -2) . '-' . substr($value, 1, 2);
		$query = '
	...
	// check if customer is in a group which is allowed to use sponsorship
	...
	static public function getStatistics($idCustomer) {
		$context = Context::getContext();
	...
	static private function _getSponsorshipsList(&$result, $idSponsor=null) {
		...
		$query = '
			...
			SELECT rs.id_sponsor, rs.id_customer AS id_sponsored, rs.channel, rs.firstname, rs.lastname, 1 AS level_sponsorship, 0, 0, 0
			FROM `'._DB_PREFIX_.'rewards_sponsorship` AS rs
			WHERE 0 = (
	...
	// return all customers from the groups allowed to be sponsor, and which are not in the descendants tree of the customer
	static public function getAvailableSponsors($id_customer) {
		// get All customers from group allowed to be sponsor
		$result = array();
		...
		if (!empty($allowed_groups)) {
			$query = '
				...
				AND id_group IN ('.Configuration::get('RSPONSORSHIP_GROUPS').')';
			$rows = Db::getInstance()->ExecuteS($query);
			if (is_array($rows)) {
				$descendants = array();
