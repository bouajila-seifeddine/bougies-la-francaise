...
class RewardsPaymentModel extends ObjectModel
{
	...
	public static function askForPayment($credits, $detail, $file) {
		...
		if (isset($file['name']) && !empty($file['tmp_name'])) {
			...
			$fileAttachment['content'] = file_get_contents($path);
		...
		$module->sendMail((int)Configuration::get('PS_LANG_DEFAULT'), 'payment-request', $module->l2('Payment request', (int)Configuration::get('PS_LANG_DEFAULT'), 'rewardspaymentmodel'), $data, strval(Configuration::get('PS_SHOP_EMAIL')), NULL, $fileAttachment);
		return true;
	}
