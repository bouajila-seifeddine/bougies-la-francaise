...
;(function($jqPm) {
	...
	$jqPm.widget("ui.sortableTree", $jqPm.extend($jqPm.ui.mouse, {
		init: function() {
		...
		propagate: function(n,e,inst) {
			$jqPm.ui.plugin.call(this, n, [e, this.ui(inst)]);
			this.element.triggerHandler(n == "sort" ? n : "sort"+n, [e, this.ui(inst)], this.options[n]);
		},
		serialize: function(o) {
			
			var items = $jqPm(this.options.items, this.element).not('.ui-sortableTree-helper'); //Only the items of the sortable itself
			var str = []; o = o || {};
			
			items.each(function() {
				var res = ($jqPm(this).attr(o.attribute || 'id') || '').match(o.expression || (/(.+)[-=_](.+)/));
		...
		toArray: function(attr) {
			...
			items.each(function() { ret.push($jqPm(this).attr(attr || 'id')); });
			return ret;
		},
		enable: function() {
		...
		refreshItems: function() {
			...
			var queries = [$jqPm(this.options.items, this.element)];
			
			if(this.options.connectWith) {
				for (var i = this.options.connectWith.length - 1; i >= 0; i--){
					var cur = $jqPm(this.options.connectWith[i]);
					for (var j = cur.length - 1; j >= 0; j--){
						var inst = $jqPm.data(cur[j], 'sortableTree');
						if(inst && !inst.options.disabled) {
							queries.push($jqPm(inst.options.items, inst.element));
			...
			for (var i = queries.length - 1; i >= 0; i--){
				queries[i].each(function() {
					$jqPm.data(this, 'sortableTree-item', true); // Data for target checking (mouse manager)
					items.push({
						item: $jqPm(this),
						width: 0, height: 0,
						left: 0, top: 0
		...
		mouseStart: function(e,el) {
			...
			var currentItem = null, nodes = $jqPm(e.target).parents().each(function() {	
				if($jqPm.data(this, 'sortableTree-item')) {
					currentItem = $jqPm(this);
					return false;
				}
			});
			if($jqPm.data(e.target, 'sortableTree-item')) currentItem = $jqPm(e.target);
			
			if(!currentItem) return false;	
			if(this.options.handle) {
				var validHandle = false;
				$jqPm(this.options.handle, currentItem).each(function() { if(this == e.target) validHandle = true; });
			...
			this.helper = typeof o.helper == 'function' ? $jqPm(o.helper.apply(this.element[0], [e, this.currentItem])) : this.currentItem.clone();
			...
			if($jqPm.ui.ddmanager) $jqPm.ui.ddmanager.current = this;
			if ($jqPm.ui.ddmanager && !o.dropBehaviour) $jqPm.ui.ddmanager.prepareOffsets(this, e);
		...
		mouseStop: function(e) {
			...
			var dropped = ($jqPm.ui.ddmanager && !this.options.dropBehaviour) ? $jqPm.ui.ddmanager.drop(this, e) : false;
		...
		mouseDrag: function(e) {
			...
			if($jqPm.ui.ddmanager) $jqPm.ui.ddmanager.drag(this, e);
			var intersectsWithDroppable = false;
			$jqPm.each($jqPm.ui.ddmanager.droppables, function() {
	...
	$jqPm.extend($jqPm.ui.sortableTree, {
		defaults: {
			items: '> *',
			zIndex: 1000,
			distance: 1
