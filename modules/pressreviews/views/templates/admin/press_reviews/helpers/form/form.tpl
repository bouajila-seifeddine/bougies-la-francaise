		...
		<fieldset id="fieldset_{$f}">
			{foreach $fieldset.form as $key => $field}
				...
				{elseif $key == 'input'}
					...
					<div {if isset($input.classTop)}class="{$input.classTop}"{/if}>
						...
						{else}
							...
							{block name="field"}
								...
								{elseif $input.type == 'thumb_img'}
								    {if isset($input.display_image) && $input.display_image}
								        {if isset($fields_value.thumb_img) && $fields_value.thumb_img}
								        <div id="image">
								                {$fields_value.thumb_img}
								                <p align="center">{l s='File size'} {$fields_value.thumb_img_size}kb</p>
								                <a href="{$current}&{$identifier}={$form_id}&token={$token}&deleteImage=1&img=thumb_img">
								                	<img src="../img/admin/delete.gif" alt="{l s='Delete'}" /> {l s='Delete'}
								                </a>
								        </div><br />
								        {/if}
								...
								{elseif $input.type == 'big_img'}
								    {if isset($input.display_image) && $input.display_image}
								        {if isset($fields_value.big_img) && $fields_value.big_img}
								        <div id="image">
								                {$fields_value.big_img}
								                <p align="center">{l s='File size'} {$fields_value.big_img_size}kb</p>
								                <a href="{$current}&{$identifier}={$form_id}&token={$token}&deleteImage=1&img=big_img">
								                	<img src="../img/admin/delete.gif" alt="{l s='Delete'}" /> {l s='Delete'}
								                </a>
								        </div><br />
								        {/if}
...
{/if}
{if $firstCall}
	<script src="{$smarty.const._MODULE_DIR_}pressreviews/views/js/jquery-ui-timepicker-addon.js" type="text/javascript"></script>
	{if {$lang_iso} != 'en'}<script src="{$smarty.const._MODULE_DIR_}pressreviews/views/js/i18n/jquery-ui-timepicker-{$lang_iso}.js" type="text/javascript"></script>{/if}
	<script type="text/javascript">
		function showInputs() {
			var selectType = $("#type");

			if(selectType[0].value == 'lightbox')
			{
				if( $('.big_img').css('display') == 'none' )
				{
					$('.big_img').slideDown('slow');
					$('.big_txt').slideDown('slow');
					$('.link').slideUp('slow');
				}
			}
			else if(selectType[0].value == 'link')
			{
				if( $('.link').css('display') == 'none' )
				{
					$('.big_img').slideUp('slow');
					$('.big_txt').slideUp('slow');
					$('.link').slideDown('slow');
				}
			}
		}
		...
		function loadInputs() {
			var selectType = $("#type");
			console.log(selectType[0].value);
			if(selectType[0].value == 'lightbox')
			{
