...
class ProductCommentsDefaultModuleFrontController extends ModuleFrontController
{
	...
	protected function ajaxProcessAddComment()
	{
		...
		if (!count($errors))
		{
			...
			{
				$result = false;
				$errors[] = $module_instance->l('You should wait', 'default').' '.Configuration::get('PRODUCT_COMMENTS_MINIMAL_TIME').' '.$module_instance->l('seconds before posting a new comment', 'default');
			}
		}
		else
			$result = false;
