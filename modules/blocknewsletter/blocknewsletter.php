...
class Blocknewsletter extends Module
{
	...
	protected function newsletterRegistration()
	{
		...
		else if ($_POST['action'] == '0')
		{
			...
			if (empty(Tools::getValue('days')) || empty(Tools::getValue('months')) || empty(Tools::getValue('years')))
				return $this->error = $this->l('All fields are madatory.');
			...
			if (!$this->isRegistered($register_status))
			{
				if (Configuration::get('NW_VERIFICATION_EMAIL'))
				{
					...
					if ($register_status == self::GUEST_NOT_REGISTERED)
						$this->registerGuest($_POST, false);
	...
	{
		$dbquery = new DbQuery();
		$dbquery->select('c.`id_customer` AS `id`, s.`name` AS `shop_name`, gl.`name` AS `gender`, c.`lastname`, c.`firstname`, c.`email`, c.`birthday`, c.`newsletter` AS `subscribed`, c.`newsletter_date_add`');
		...
		$dbquery = new DbQuery();
		$dbquery->select('CONCAT(\'N\', n.`id`) AS `id`, s.`name` AS `shop_name`, NULL AS `gender`, n.`lastname`, n.`firstname`, n.`birthday`, n.`email`, n.`active` AS `subscribed`, n.`newsletter_date_add`');
	...
	{
		if ($register_status == self::GUEST_NOT_REGISTERED)
			return $this->registerGuest($_POST);
	 ...
	 * @return bool
	 */
	protected function registerGuest($data, $active = true)
	{
		$birthday = $data['years'] . '-' . $data['months'] . '-' . $data['days'];
		$sql = 'INSERT INTO '._DB_PREFIX_.'newsletter (id_shop, id_shop_group, email, firstname, lastname, birthday, newsletter_date_add, ip_registration_newsletter, http_referer, active)
				VALUES
	...
	public function export_csv()
	{
		...
		if ($result)
		{
			...
			{
				$header = array('id', 'shop_name', 'gender', 'lastname', 'firstname', 'email', 'birthday', 'subscribed', 'subscribed_on');
	...
	private function getCustomers()
	{
		...
		{
			$dbquery = new DbQuery();
			$dbquery->select('CONCAT(\'N\', n.`id`) AS `id`, s.`name` AS `shop_name`, NULL AS `gender`, n.`lastname`, n.`firstname`, n.`email`, n.`birthday`, n.`active` AS `subscribed`, n.`newsletter_date_add`');
			$dbquery->from('newsletter', 'n');
			$dbquery->leftJoin('shop', 's', 's.id_shop = n.id_shop');
			$dbquery->where('n.`active` = 1');
			if ($id_shop)
