...
class LengowImportAbstract
{
	...
	public function exec()
	{
		...
		{  	
			...
			$result = LengowImport::getOrdersFromApi($this->order_id, $this->marketplace_name, $this->date_from, $this->date_to);
			$total_orders = count($result);
			if ($this->order_id)
				LengowCore::log($total_orders.' order found for order ID: '.$this->order_id.' and markeplace: '.$this->marketplace_name.' with account ID: '.LengowCore::getIdAccount(), $this->log_output);
			else
			{	
				$marketplaces = LengowCore::getImportMarketplaces();
				$marketplace_names = false;
				foreach ($marketplaces as $marketplace)
					$marketplace_names .= (!$marketplace_names ? $marketplace : ', '.$marketplace);
				LengowCore::log($total_orders.' order'.($total_orders > 1 ? 's ' : ' ').'found for markeplaces: '.$marketplace_names.' with account ID: '.LengowCore::getIdAccount(), $this->log_output);
			}
			if ($total_orders <= 0)
			{
				LengowImport::setEnd();
	...
	protected function getOrdersFromApi($order_id, $marketplace_name, $date_from, $date_to)
	{
		...
		if (LengowCheck::isValidAuth())
		{
			...
			if ($order_id && $marketplace_name)
				LengowCore::log('Connector: get order with order id: '.$order_id.' and marketplace: '.$marketplace_name, $this->log_output);
			else
				LengowCore::log(
			...
			do 
			{
				...
				if (isset($results->error))
					throw new LengowImportException('Lengow webservice : '.$results->error->code.' - '.$results->error->message);
				// Construct array orders
				foreach ($results->results as $order)
					if (in_array((string)$order->marketplace, LengowCore::getImportMarketplaces()))
						$orders[] = $order;
				$page++;
			} 
	...
	protected static function importOrders($orders, $id_lang, $id_shop, $debug = false, $force_product = true, $limit = 0, $log_output = false, $force_price = true)
	{
		...
		{	
			...
			foreach ($order_data->packages as $package)
			{
				...
				{
					LengowCore::log('create order fail: no delivery address in the order', $log_output, $lengow_id);
					continue;
				}
				...
				$message = LengowLog::loadLogInfo($lengow_id, $delivery_address_id);
				if ($message)
				{
				...
				// check order data
				if (!LengowImport::checkOrderData($order_data, $package, $lengow_id, $delivery_address_id, $debug))
					continue;

				// update order state if already imported
				$order_id = LengowOrder::getOrderIdFromLengowOrders($lengow_id, $delivery_address_id, (string)$marketplace->name);
				...
				if ($order_id)
				{
					...
					else
					{
						try
						{
							if ($order->updateState($marketplace, $order_state, (count($package->delivery->trackings) > 0 ? (string)$package->delivery->trackings[0]->number : null)))
							{
				...
				try
				{
					...
					if (!is_null($external_ids) && count($external_ids) > 0)
					{
						...
						{
							$result = LengowOrder::getLengowIdFromLengowDeliveryAddress((integer)$external_id, (integer)$delivery_address_id);
							if ($result)
							{
					...
					if ($id_order_prestashop && !$debug && !$is_reimport)
					{
						$message = 'already imported in Prestashop with order ID '.$id_order_prestashop;
						LengowLog::addLog($order_data, $lengow_id, $delivery_address_id, $message, 1);
						LengowCore::log($message, $log_output, $lengow_id);
						continue;
					}
					...
					{
						$domain = !LengowCore::getHost() ? 'prestashop.shop' : LengowCore::getHost();
						$billing_data['email'] = 'generated-email+'.$lengow_id.'@'.$domain;
					...
					// billing
					$billing_address = LengowImport::getAddress($customer->id, $billing_data);
					...
					$shipping_address = LengowImport::getAddress($customer->id, $shipping_data, (!is_null($trackings) ? $trackings[0] : null));
					...
					else
					{
						...
						foreach ($order_list as $order)
						{
							...
							if (!LengowImport::addLengowOrder($lengow_id, $delivery_address_id, $order, $order_data, $package, $order_amount))
						...
						if (!$debug)
						{
							...
							if (count($order_ids) > 0)
							{
								...
								if (is_null($result) || (isset($result['detail']) && $result['detail'] == 'Pas trouvé.') || isset($result['error']))
									LengowCore::log('WARNING ! Order could NOT be synchronised with Lengow webservice (ID '.$order->id.')', $debug, $lengow_id);
								else 
									LengowCore::log('order successfully synchronised with Lengow webservice (ID '.$order->id.')', $debug, $lengow_id);
							}
						}
						LengowLog::addLog($order_data, $lengow_id, $delivery_address_id, $success_message, 1);
				...
				if (isset($error_message))
				{
					...
					LengowLog::addLog($order_data, $lengow_id, $delivery_address_id, $error_message);
					unset($error_message);
				}
	 ...
	 * @return boolean
	 */
	protected static function checkOrderData($order_data, $package, $lengow_id, $delivery_address_id, $debug)
	{
		$error_message = false;

		if (count($package->cart) == 0)	
			$error_message = 'create order fail: no product in the order';
		elseif (is_null($order_data->currency))
			$error_message = 'no currency in the order';
		elseif ($order_data->total_order == -1)
			$error_message = 'prices can\'t be calculated in the correct currency';
	 ...
	 */
	protected static function getAddress($id_customer, $address_data = array(), $tracking_informations = null)
	{
		...
		if ($address)
		{
			if ($address->id_customer == $id_customer && $address->lastname == $address_data['last_name'] && $address->firstname == $address_data['first_name'])
			{
	...
	protected static function getProducts($cart_data, $marketplace, $lengow_id, $id_shop)
	{
		...
		foreach ($cart_data as $product)
		{
			...
			if (!is_null($product_data['marketplace_status']))
			{
				...
				if ($state_product == 'canceled' || $state_product == 'refused')
				{
					LengowCore::log('product '.$product_data['merchant_product_id']->id.' could not be added to cart - status: '.$state_product, false, $lengow_id);
					continue;
				}
			}
			$ids = false;
			...
			if (!$found) 
				throw new Exception(
					'product '
					.(!is_null($product_data['merchant_product_id']->id) ? (string)$product_data['merchant_product_id']->id : (string)$product_data['marketplace_product_id'] )
					.' could not be found'
				);
		}
		return $products;
	 ...
	 * @return boolean
	 */
	protected static function addLengowOrder($lengow_id, $delivery_address_id, $order, $order_data, $package_data, $order_amount)
	{
		if (_PS_VERSION_ >= '1.5')
			return Db::getInstance()->insert(
										...
										array(
											...
											'delivery_address_id'	=> pSQL((int)$delivery_address_id),
											...
											'carrier'				=> pSQL(count($package_data->delivery->trackings) > 0 ? (string)$package_data->delivery->trackings[0]->carrier : null),
											'method'				=> pSQL(count($package_data->delivery->trackings) > 0 ? (string)$package_data->delivery->trackings[0]->method : null),
											'tracking'				=> pSQL(count($package_data->delivery->trackings) > 0 ? (string)$package_data->delivery->trackings[0]->number : null),
											'sent_marketplace'		=> pSQL(count($package_data->delivery->trackings) > 0 ? ($package_data->delivery->trackings[0]->is_delivered_by_marketplace == null ? 0 : 1) : 0),
