...
/**
 * The Lengow Import Class V2
 *
 * @author Team Connector <team-connector@lengow.com>
 * @copyright 2015 Lengow SAS
 */
class LengowImportV2Abstract {
	...
	public function exec()
	{
		...
		{  	// start of actual import process
			...
			$result = LengowImportV2::getOrdersFromApi(
													$this->order_id,
													$this->feed_id,
													$this->date_from,
													$this->date_to
	...
	{
		LengowImportV2::$processing = true;
	...
	{
		$connector  = new LengowConnectorV2((integer)LengowCore::getIdCustomer(), LengowCore::getTokenCustomer());
		...
		if (!$orders || !isset($orders->orders))
			throw new LengowApiExceptionV2('no order data retrieved from API - '.Tools::jsonEncode($orders), 4);
		return $orders;
	}
	...
	{
		LengowImportV2::$processing = false;
	...
	protected static function importOrders($orders, $id_lang, $id_shop, $debug = false, $force_product = true, $limit = 0, $log_output = false, $force_price = true)
	{
		...
		foreach ($orders->order as $order_data)
		{
			...
			$message = LengowLog::loadLogInfo($lengow_id);
			if ($message)
			{
			...
			LengowImportV2::$current_order = $lengow_id;
			$feed_id = (string)$order_data->idFlux;
			// check order status
			$marketplace = LengowCore::getMarketplaceSingletonV2((string)$order_data->marketplace);
			$order_state = (string)$order_data->order_status->marketplace;

			// update order state if already imported
			$order_id = LengowOrder::getOrderIdFromLengowOrdersV2($lengow_id, $feed_id, $marketplace->name);
			if ($order_id)
			{
				...
				else
				{
					try
					{
						...
						if ($order->updateStateV2($marketplace, $api_state, (string)$order_data->tracking_informations->tracking_number))
						{
							$available_states = LengowCore::getOrderStates($id_lang);
							$state_name = '';
							foreach ($available_states as $state)
							{
								if ($state['id_order_state'] === LengowCore::getOrderStateV2($marketplace->getStateLengow($api_state)))
			...
			if (!LengowImportV2::checkState($order_state, $marketplace))
			{
			...
			try
			{
				...
				if (!empty($external_id) && !$debug && !$is_reimport)
				{
					...
					LengowLog::addLog($order_data, $lengow_id, null, $message, 1);
					LengowCore::log($message, $log_output, $lengow_id);
					continue;
				}
				...
				if ($force_price)
					$order_data = LengowImportV2::rewriteData($order_data, Configuration::get('LENGOW_IMPORT_PROCESSING_FEE'));
				...
				// get billing data
				$billing_data = LengowAddress::extractAddressDataFromAPIV2($order_data->billing_address, LengowAddress::BILLING);
				...
				$customer = LengowImportV2::getCustomer($billing_data);
				if (!$customer->id)
				...
				// billing
				$billing_address = LengowImportV2::getAddress($customer->id, $billing_data);
				...
				// shipping
				$shipping_data = LengowAddress::extractAddressDataFromAPIV2($order_data->delivery_address, LengowAddress::SHIPPING);
				$shipping_address = LengowImportV2::getAddress($customer->id, $shipping_data, $order_data->tracking_informations);
				...
				// get carrier
				$cart_data['id_carrier'] = LengowImportV2::getCarrierId($order_data->tracking_informations, $marketplace, $lengow_id, $id_lang, $shipping_address);
				...
				$products = LengowImportV2::getProducts($order_data->cart, $marketplace, $lengow_id, $id_shop);
				...
				else
				{
					...
					foreach ($order_list as $order)
					{
						...
						if (!LengowImportV2::addLengowOrder($lengow_id, $feed_id, $order, $order_data))
					...
					LengowLog::addLog($order_data, $lengow_id, null, $success_message, 1);
				...
				if ($shipped_by_mp)
				{
					...
					LengowImportV2::addQuantityBack($products, $id_shop);
				}
			}
			...
			catch (LengowImportExceptionV2 $lie)
			{
			...
			if (isset($error_message))
			{
				...
				LengowLog::addLog($order_data, $lengow_id, null, $error_message);
				unset($error_message);
			}
			LengowImportV2::$current_order = -1;
	...
	protected static function getCustomer($customer_data = array())
	{
		...
		// create new customer
		$customer->assignV2($customer_data);
		return $customer;
	}
	...
	protected static function getAddress($id_customer, $address_data = array(), $tracking_informations = null)
	{
		...
		if ($address)
		{
			if ($address->id_customer == $id_customer && $address->lastname == $address_data['lastname'] && $address->firstname == $address_data['firstname'])
			{
	...
	protected static function getProducts($cart_data, $marketplace, $lengow_id, $id_shop)
	{
		...
		{
			$product_data = LengowProduct::extractProductDataFromAPIV2($product);
			...
			foreach ($product_ids as $attribute_name => $attribute_value)
			{
				...
				if (!empty($ids))
				{
					...
					if (!isset($ids['id_product_attribute']))
					{
						...
						if ($p->hasAttributes())
							throw new LengowImportExceptionV2('product '.$p->id.' is a parent ID. Product variation needed');
					}
					...
					if (array_key_exists($id_full, $products))
					{
						...
						$products[$id_full]['price'] += (float)$product_data['price'];
					} 
					else 
	 ...
	 * @param LengowMarketplaceV2	$marketplace 	order marketplace
	 *
	 * @return boolean
	 */
	 ...
	 * @return boolean
	 */
	protected static function addLengowOrder($lengow_id, $feed_id, $order, $order_data)
	{
		if (_PS_VERSION_ >= '1.5')
			return Db::getInstance()->insert(
											...
											array(
												...
												'message'			=> pSQL((string)$order_data->order_comments),
												'total_paid'		=> (float)$order_data->order_amount,
												'carrier'			=> pSQL((string)$order_data->tracking_informations->tracking_carrier),
												'method'			=> pSQL((string)$order_data->tracking_informations->tracking_method),
												'tracking'			=> pSQL((string)$order_data->tracking_informations->tracking_number),
												'sent_marketplace'	=> pSQL((string)$order_data->tracking_informations->tracking_deliveringByMarketPlace),
	 ...
	 * @return mixed
	 */
	public static function getCarrierId($tracking_informations, LengowMarketplaceV2 $marketplace, $lengow_id, $id_lang, $shipping_address)
	{
		$carrier_id = false;
 ...
 */
class LengowImportExceptionV2 extends Exception
{
