...
try {
	if (!function_exists('curl_init'))
		throw new LengowApiExceptionV2('Lengow needs the CURL PHP extension.', -1);
	if (!function_exists('json_decode'))
		throw new LengowApiExceptionV2('Lengow needs the JSON PHP extension.', -2);
	if (!function_exists('simplexml_load_string'))
		throw new LengowApiExceptionV2('Lengow needs the SIMPLE XML PHP extension.', -3);
} catch (LengowApiExceptionV2 $e) {
	echo $e->getMessage();
}

/**
 * The Lengow connector API V2
 *
 * @author Team Connector <team-connector@lengow.com>
 * @copyright 2015 Lengow SAS
 */
class LengowConnectorV2Abstract {
	...
	public function __construct($id_customer, $token)
	{
		try {
			if (is_integer($id_customer))
				...
				throw new LengowApiExceptionV2('Error Lengow Customer ID', 1);
			if (Tools::strlen($token) > 10)
				$this->token = $token;
			else
				throw new LengowApiExceptionV2('Error Lengow Token API', 2);
		} catch (LengowApiExceptionV2 $e) {
			$this->error = $e;
			return false;
		}
	...
	*/
	public function api($method, $array = array())
	{
		if (!$api = $this->getMethod($method))
			throw new LengowApiExceptionV2('Error unknown API method', 3);
		else
		{
			try
			{
	...
	*/
	private function callAction($service, $method, $array)
	{
		...
		$result = LengowConnectorV2::makeRequest($url);
		return LengowConnectorV2::format($result, self::$DOMAIN_LENGOW[$service]['format']);
	}
	...
	*/
	protected static function makeRequest($url)
	{
		...
		if ($result === false)
		{
			...
			throw new LengowApiExceptionV2(
		...
		if (is_object(Tools::jsonDecode($result)))
			if (Tools::strtolower(Tools::jsonDecode($result)->return) == 'ko')
			{
				...
				throw new LengowApiExceptionV2(Tools::jsonDecode($result)->error, 4);
			}
		return $result;
	}
 ...
 */
class LengowApiExceptionV2 extends Exception {
