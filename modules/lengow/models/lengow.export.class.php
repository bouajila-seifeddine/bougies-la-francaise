	...
	*/
	...
	public function export($products, $fields, $shop)
	{
		...
		foreach ($products as $p)
		{
			...
			foreach ($fields as $field)
			{
				...
				if ($this->export_combinations && $product->hasAttributes())
				{
					...
					if (empty($combinations))
						throw new LengowExportException('Unable to retrieve product combinations for product '.$p['id_product']);
			...
			// Get additional data
			$product_data = $this->setAdditionalFieldsValues($product, null, $product_data);
			...
			{
				foreach ($combinations_data as $key => $combination_data)
				{
					// get additional data for combination
					$id_product_attribute = explode('_', $key);
					$combination_data = $this->setAdditionalFieldsValues($product, $id_product_attribute[1], $combination_data);
	...
	protected function getFields()
	{
		...
		if (is_array($export_fields))
			...
			foreach (LengowExport::$DEFAULT_FIELDS as $key => $field)
				$fields[] = $key;
